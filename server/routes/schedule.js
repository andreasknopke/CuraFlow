...
router.post('/export', async (req, res, next) => {
  try {
    ...
    for (const block of blockRows) {
      const dateStr =
        block.date instanceof Date
          ...
          : String(block.date).substring(0, 10);
    ...
    const sections = [
      ...
      {
        title: 'Dienste',
        rows: sortedWorkplaces.filter((w) => w.category === 'Dienste').map((w) => w.name),
      },
      {
        title: 'Rotationen',
        rows: sortedWorkplaces.filter((w) => w.category === 'Rotationen').map((w) => w.name),
      },
      {
        title: 'Demonstrationen & Konsile',
        rows: sortedWorkplaces
    ...
    const findContent = (posName, dateStr) => {
      if (posName === 'Sonstiges') {
        const note = noteRows.find(
          (n) => normalizeDate(n.date) === dateStr && n.position === posName,
        );
    ...
    sections.forEach((section) => {
      ...
      section.rows.forEach((rowName) => {
        ...
        const posSetting = colorSettings.find(
          (s) => s.name === rowName && s.category === 'position',
        );
        if (posSetting) {
    ...
    // Return as JSON with base64 file for the frontend download helper.
