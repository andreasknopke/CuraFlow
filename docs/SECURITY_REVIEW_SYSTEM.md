...
**Severity:** HIGH — **substantially mitigated (2026-07-29); full structural fix nearly complete (Phase 1)**
> The structural primitive is in place: a Kysely adapter (`server/utils/db.js`, `createKysely` + mysql2/promise callback bridge) routes identifiers through central escaping. Migrated so far: `getValidColumns` (PR 1.0 spike), the **generic create INSERT** path (PR 1.1, `insertRow`), the **update/delete/read-back** paths (PR 1.2, `updateRow`/`deleteRow`/`selectRow`), and the **list/filter/get** paths (PR 1.3, `filterRows` — which closes the previously-unvalidated **filter-key** interpolation hole via `sql.ref`). Remaining hand-built backtick-interpolation sites: `bulkCreate` (transactional, deferred pending e2e coverage) and `atomic.js` (PR 1.4). Until then `assertValidIdentifier` at the route entry remains the live guard. S1 is closed only when the per-PR grep gate shows zero residual hand-interpolated identifiers.
...
- 🔧 **S1 structural primitive (Phase 1 PRs 1.0–1.3)** — Kysely adapter `server/utils/db.js` (`createKysely` + `mysql2/promise` callback bridge) routes identifiers through central escaping; `getValidColumns` (PR 1.0), the generic create path (PR 1.1, `insertRow`), the update/delete/read-back paths (PR 1.2, `updateRow`/`deleteRow`/`selectRow`), and the list/filter/get paths (PR 1.3, `filterRows`) migrated. **The filter-key injection hole is now closed** (every filter key escaped via `sql.ref`). S1 itself stays open until bulkCreate (deferred) and atomic.js (PR 1.4) migrate (per-PR grep gate).
