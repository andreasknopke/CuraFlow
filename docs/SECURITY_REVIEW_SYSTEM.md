...
**Severity:** HIGH — **nearly closed (2026-07-29); only bulkCreate remains (Phase 1)**
> The structural primitive is in place: a Kysely adapter (`server/utils/db.js`, `createKysely` + mysql2/promise callback bridge) routes identifiers through central escaping. Migrated so far: `getValidColumns` (PR 1.0 spike), the **generic create INSERT** path (PR 1.1, `insertRow`), the **update/delete/read-back** paths (PR 1.2, `updateRow`/`deleteRow`/`selectRow`), the **list/filter/get** paths (PR 1.3, `filterRows` — closed the dbProxy filter-key hole), and **`atomic.js`** (PR 1.4 — all 5 helpers, closing atomic's filter-key hole too). The **only remaining hand-built backtick-interpolation site is `bulkCreate`** (transactional, deferred pending a bulkCreate e2e test). Until then `assertValidIdentifier` at the route entry remains the live guard for that one path. S1 is closed when bulkCreate migrates.
...
- 🔧 **S1 structural primitive (Phase 1 PRs 1.0–1.4)** — Kysely adapter `server/utils/db.js` (`createKysely` + `mysql2/promise` callback bridge) routes identifiers through central escaping; `getValidColumns` (PR 1.0), the generic create path (PR 1.1, `insertRow`), update/delete/read-back paths (PR 1.2), list/filter/get paths (PR 1.3, `filterRows`), and `atomic.js`'s 5 helpers (PR 1.4) all migrated. **Both filter-key injection holes (dbProxy + atomic) are closed** via `sql.ref`. The only remaining hand-built interpolation site is `bulkCreate` (deferred pending a bulkCreate e2e test); S1 closes when it migrates.
