...
**Severity:** HIGH — **largely mitigated (2026-07-29); full structural fix in progress (Phase 1)**
> The structural primitive is in place: a Kysely adapter (`server/utils/db.js`, `createKysely` + mysql2/promise callback bridge) routes identifiers through central escaping. Migrated so far: `getValidColumns` (PR 1.0 spike), the **generic create INSERT** path (PR 1.1, `insertRow`), and the **update/delete/read-back** paths (PR 1.2, `updateRow`/`deleteRow`/`selectRow` + the update-path ShiftEntry reinsert via `insertRow`). Remaining hand-built backtick-interpolation sites: the list/filter path (PR 1.3), `bulkCreate` (transactional, deferred pending e2e coverage), and `atomic.js` (PR 1.4). Until then `assertValidIdentifier` at the route entry remains the live guard. S1 is closed only when the per-PR grep gate shows zero residual hand-interpolated identifiers.
...
- 🔧 **S1 structural primitive (Phase 1 PRs 1.0–1.2)** — Kysely adapter `server/utils/db.js` (`createKysely` + `mysql2/promise` callback bridge) routes identifiers through central escaping; `getValidColumns` (PR 1.0), the generic create path (PR 1.1, `insertRow`), and the update/delete/read-back paths (PR 1.2, `updateRow`/`deleteRow`/`selectRow`) migrated. S1 itself stays open until the list/filter path (PR 1.3), bulkCreate (deferred), and atomic.js (PR 1.4) migrate (per-PR grep gate).
