...
> The structural primitive is in place: a Kysely adapter (`server/utils/db.js`, `createKysely` + mysql2/promise callback bridge) routes identifiers through central escaping. Migrated so far: `getValidColumns` (PR 1.0 spike) and the **generic create INSERT** path (PR 1.1, via `insertRow` → `insertInto`). Remaining hand-built backtick-interpolation sites: the update/delete paths, the list/filter path, `bulkCreate` (transactional, deferred pending e2e coverage), the `:1276` update-path ShiftEntry reinsert, and `atomic.js` — migrated in Phase 1 PRs 1.2–1.4. Until then `assertValidIdentifier` at the route entry remains the live guard. S1 is closed only when the per-PR grep gate shows zero residual hand-interpolated identifiers.
...
- 🔧 **S1 structural primitive (Phase 1 PRs 1.0–1.1)** — Kysely adapter `server/utils/db.js` (`createKysely` + `mysql2/promise` callback bridge) routes identifiers through central escaping; `getValidColumns` (PR 1.0) and the generic create INSERT path (PR 1.1, `insertRow`) migrated. S1 itself stays open until the update/delete/list-filter + bulkCreate + atomic.js sites migrate in Phase 1 PRs 1.2–1.4 (per-PR grep gate).
