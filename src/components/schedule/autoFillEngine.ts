...
interface GenerateSuggestionsParams {
  ...
  getDoctorQualIds: (id: number) => number[];
  getWpRequiredQualIds: (id: number) => number[];
  getWpOptionalQualIds?: (id: number) => number[];
  getWpExcludedQualIds?: (id: number) => number[];
  getWpDiscouragedQualIds?: (id: number) => number[];
...
type SuggestionResult = ShiftEntry[] & { __debug?: { requestId: string; entries: DebugEntry[] } };

export function generateSuggestions({
  weekDays,
  doctors: doctorsParam,
  workplaces,
  existingShifts,
  allShifts = [],
  trainingRotations,
  ...
  const debugEntries: DebugEntry[] = debug?.entries || [];
  ...
  const doctorNameById: Record<number, string> = Object.fromEntries(
  ...
  const getActiveDays = (wp: Workplace): number[] =>
    wp.active_days?.length ? wp.active_days : DEFAULT_ACTIVE_DAYS;

  const isActiveOnDate = (wp: Workplace, date: Date): boolean => {
  ...
  const isQualified = (doctorId: number, wpId: number): boolean => {
  ...
  const isExcluded = (doctorId: number, wpId: number): boolean => {
  ...
  const _hasAnyOptionalQual = (doctorId: number, wpId: number): boolean => {
    const opt = getWpOptionalQualIds?.(wpId);
  ...
  const hasOptionalQualReq = (wp: Workplace): boolean => {
  ...
  const isDiscouraged = (doctorId: number, wpId: number): boolean => {
  ...
  const allowsMultiple = (wp: Workplace): boolean => workplaceAllowsMultiple(wp, customCategories);

  /** optimal_staff: target headcount */
  const getOptimal = (wp: Workplace): number => {
    if (!allowsMultiple(wp)) return 1;
    return Math.max(wp.optimal_staff ?? 1, wp.min_staff ?? 1);
  };

  const getMinStaff = (wp: Workplace): number => {
    if (!allowsMultiple(wp)) return 1;
  ...
  const getTimeslots = (wp: Workplace): WorkplaceTimeslot[] => {
  ...
  const slotKey = (posName: string, timeslotId: number | string | null | undefined): string =>
    timeslotId ? `${posName}::${timeslotId}` : posName;

  /** Count existing shifts for a position+timeslot combo on a date */
  const _countShiftsForSlot = (
    dateStr: string,
    posName: string,
    timeslotId: number | string | null,
    existingArr: ShiftEntry[],
    suggestionsArr: ShiftEntry[],
  ): number => {
    let count = 0;
  ...
  const getWeekly = (id: number): number => weeklyCount[id] || 0;
  const incWeekly = (id: number): void => {
  ...
  // Read limit settings
  const getSetting = (key: string, def: string): number => {
  ...
  const foregroundPositions = new Set<string>(
  ...
  const getServiceType = (positionName: string): 'fg' | 'bg' | 'other' => {
  ...
  const serviceHistory: Record<number, { fg: number; bg: number; weekend: number }> = {};
  const getServiceHist = (docId: number): { fg: number; bg: number; weekend: number } => {
  ...
  const getDoctorFte = (docId: number): number => {
    const doc = doctors.find((d) => d.id === docId);
    return doc?.fte ?? 1.0;
  };

  /** Would assigning this service to this doctor exceed the 4-week limit? */
  const wouldExceedLimit = (docId: number, serviceName: string, dateStr: string): boolean => {
  ...
  const recordServiceAssignment = (docId: number, serviceName: string, dateStr: string): void => {
    const h = getServiceHist(docId);
    const svcType = getServiceType(serviceName);
    if (svcType === 'fg') {
      h.fg++;
   ...
   */
  const _getFairnessScore = (docId: number, serviceName: string): number => {
    const h = getServiceHist(docId);
  ...
  // ---- Rotation mapping ----
  const getActiveRotationTargets = (doctorId: number, dateStr: string): string[] => {
  ...
  const displacementCount: Record<number, number> = {};
  const _getDisplaced = (id: number): number => displacementCount[id] || 0;
  const addDisplacement = (id: number): void => {
    displacementCount[id] = (displacementCount[id] || 0) + 1;
  };

  // ---- Auto-Frei tracking across days ----
  // Doctors who get Auto-Frei on a specific date are blocked that day.
  const autoFreiByDate: Record<string, Set<number>> = {};
  const autoFreiSuggestions: ShiftEntry[] = [];
  ...
  const generateAutoFrei = (doctorId: number, dateStr: string): void => {
  ...
  const _getServiceWish = (doctorId: number, dateStr: string): Wish | undefined => {
    return wishes.find(
      (w) =>
  ...
  const hasApprovedNoService = (doctorId: number, dateStr: string): boolean => {
    return wishes.some(
      (w) =>
        w.doctor_id === doctorId &&
        w.date === dateStr &&
  ...
  const _hasPendingNoService = (doctorId: number, dateStr: string): boolean => {
    return wishes.some(
      (w) =>
        w.doctor_id === doctorId &&
        w.date === dateStr &&
  ...
  //  Classify workplaces
  // ========================================================
  const isServiceWp = (wp: Workplace): boolean => wp.category === 'Dienste';
  const isRotationWp = (wp: Workplace): boolean => wp.category === 'Rotationen';
  const isAffectsAvailability = (wp: Workplace): boolean => wp.affects_availability !== false;
  const isRotationTraineeForWp = (doctorId: number, wpName: string, dateStr: string): boolean => {
  ...
  const getConsecutiveMode = (wp: Workplace): string => {
  ...
  const wouldViolateConsecutive = (docId: number, svcName: string, dateStr: string): boolean => {
  ...
  for (const day of weekDays) {
    ...
    const baseBlocked = new Set<number>();
    ...
    for (const s of existingShifts) {
      ...
      if (wp?.category === 'Demonstrationen & Konsile' && wp?.affects_availability !== true)
        continue;
      baseBlocked.add(s.doctor_id);
    }

    // Track who is used today (starts as copy of baseBlocked)
    const usedToday = new Set<number>(baseBlocked);

    // All workplaces active today in the categories we're filling
    const allTodayWps = workplaces.filter(
      (wp) => categoriesToFill.includes(wp.category!) && isActiveOnDate(wp, day),
    );

    // Track doctors assigned to a service today (even if rotation-OK)
    const serviceAssignedToday = new Set<number>();
    for (const s of existingShifts) {
      if (s.date !== dateStr) continue;
      const wp = workplaces.find((w) => w.name === s.position);
    ...
    const assign = (
      docId: number,
      wpName: string,
      timeslotId: number | string | null = null,
    ): void => {
      const suggestion: ShiftEntry = {
        date: dateStr,
        position: wpName,
        doctor_id: docId,
        isPreview: true,
    ...
    const _hasQualCoverage = (wp: Workplace): boolean => {
      for (const s of existingShifts) {
    ...
    // ============================================================
    const rotationImpactScore: Record<number, number> = {};
    {
      /** Compute impact scores for a given day/date, considering who is blocked */
      const computeImpactForDay = (
        targetDay: Date,
        blockedSet: Set<number>,
        multiplier: number,
      ): void => {
        const critWps = workplaces.filter(
          (wp) =>
            !isServiceWp(wp) &&
        ...
        for (const wp of critWps) {
          const qualifiedPool = doctors.filter(
            (d) => !blockedSet.has(d.id) && !isExcluded(d.id, wp.id) && isQualified(d.id, wp.id),
          );
      ...
      if (hasAutoOffService) {
        ...
        if (nextDateStr) {
          ...
          const nextDayBlocked = new Set<number>();
          ...
          for (const s of existingShifts) {
            ...
            // Demos/Konsile only block if they are marked as availability-relevant
            if (xwp?.category === 'Demonstrationen & Konsile' && xwp?.affects_availability !== true)
              continue;
            nextDayBlocked.add(s.doctor_id);
          }
    ...
    const _getRotationImpact = (docId: number): number => rotationImpactScore[docId] || 0;
    ...
    const serviceBlocked = new Set<number>(usedToday);

    // ============================================================
    //  PHASE B: Verfügbarkeitsrelevante Arbeitsplätze (non-service)
    // ============================================================
    const availWps = allTodayWps.filter((wp) => !isServiceWp(wp) && isAffectsAvailability(wp));
    ...
    if (availWps.length > 0) {
      ...
      const qualSlots = availSlots
        ...
        .sort((a, b) => {
          const aPool = doctors.filter(
            (d) => !usedToday.has(d.id) && !isExcluded(d.id, a.wp.id) && isQualified(d.id, a.wp.id),
          ).length;
          const bPool = doctors.filter(
            (d) => !usedToday.has(d.id) && !isExcluded(d.id, b.wp.id) && isQualified(d.id, b.wp.id),
          ).length;
          return aPool - bPool;
        });
      ...
      for (const slot of qualSlots) {
        ...
        const hasSlotCoverage = (): boolean => {
          for (const s of existingShifts) {
            if (s.date === dateStr && s.position === wp.name && isQualified(s.doctor_id, wp.id)) {
              if (tsId) {
                if (s.timeslot_id === tsId) return true;
              } else {
                if (!s.timeslot_id) return true;
              }
            }
          }
          for (const s of suggestions) {
        ...
        const sortB1 = (a: Doctor, b: Doctor): number => {
          const aRotElsewhere = getActiveRotationTargets(a.id, dateStr).some((t) => t !== wp.name)
            ? 1
            : 0;
          const bRotElsewhere = getActiveRotationTargets(b.id, dateStr).some((t) => t !== wp.name)
            ? 1
            : 0;
        ...
        let eligible = doctors.filter(
          (d) => !usedToday.has(d.id) && !isExcluded(d.id, wp.id) && isQualified(d.id, wp.id),
        );
      ...
      while (changed) {
        ...
        if (
          eligible.length > 0 &&
          costFn.assignmentCost(eligible[0].id, targetWp, dateStr, costContextB) < Infinity
        ) {
          ...
          debugLog(
            'phase:B:rotation-skip',
            'Skipped second occupancy for rotation workplace (no eligible trainee after filters)',
            {
              date: dateStr,
              workplace: targetWp.name,
              timeslotId: targetTsId,
      ...
      while (overChanged) {
        ...
        for (const doc of remaining) {
          ...
          {
            const withPreferred = eligibleWps.filter(
              (o) => !hasOptionalQualReq(o.wp) || hasOptionalQuals(doc.id, o.wp.id),
            );
      ...
      // ============================================================
      {
        ...
        if (phaseBSuggestions.length >= 2) {
            ...
            for (let i = 0; i < phaseBSuggestions.length && !swapImproved; i++) {
              for (let j = i + 1; j < phaseBSuggestions.length && !swapImproved; j++) {
                ...
                if (swappedCost < currentCost - 0.5) {
                  debugLog(
                    'phase:B:swap',
                    `Swap reduces cost by ${(currentCost - swappedCost).toFixed(1)}`,
                    {
          ...
          if (swapRounds > 1) {
            debugLog(
              'phase:B:swap-summary',
              `Swap optimization completed in ${swapRounds} rounds`,
              {
                date: dateStr,
    ...
    // ============================================================
    const soleOccupantDoctors = new Set<number>();
    {
      const wpStaffing: Record<string, Set<number>> = {};
      for (const s of existingShifts) {
        if (s.date !== dateStr) continue;
    ...
    // ============================================================
    const nonAvailWps = allTodayWps.filter((wp) => !isServiceWp(wp) && !isAffectsAvailability(wp));
    ...
    if (nonAvailWps.length > 0) {
      ...
      const phaseC_blocked = new Set<number>(serviceBlocked);

      const phaseCSlotCount: Record<string, number> = { ...slotCount };

      const phaseCAssignments: Record<number, Set<string>> = {};

      const isAlreadyAssignedToSlot = (
        docId: number,
        wpName: string,
        tsId: number | string | null,
      ): boolean => {
      ...
      const assignC = (
        docId: number,
        wpName: string,
        wpHasQualReq: boolean,
        tsId: number | string | null = null,
      ): void => {
        const suggestion: ShiftEntry = {
          date: dateStr,
          position: wpName,
      ...
      const qualSlotsC = nonAvailSlots
        ...
        .sort((a, b) => {
          ...
          const aPool = doctors.filter(
            (d) =>
              !serviceBlocked.has(d.id) && !isExcluded(d.id, a.wp.id) && isQualified(d.id, a.wp.id),
          ).length;
          const bPool = doctors.filter(
            (d) =>
              !serviceBlocked.has(d.id) && !isExcluded(d.id, b.wp.id) && isQualified(d.id, b.wp.id),
          ).length;
          return aPool - bPool;
        });
      ...
      while (changedC) {
        ...
        if (targetHasQualReq) {
          ...
          let eligiblePflicht = doctors.filter(
            (d) =>
              !serviceBlocked.has(d.id) &&
              !phaseC_blocked.has(d.id) &&
              !isExcluded(d.id, targetWpC.id) &&
          ...
          if (eligiblePflicht.length === 0) {
            let eligibleMehr = doctors.filter(
              (d) =>
                !serviceBlocked.has(d.id) &&
                !isExcluded(d.id, targetWpC.id) &&
                isQualified(d.id, targetWpC.id) &&
          ...
          if (eligiblePflicht.length > 0) {
            ...
            debugLog(
              'phase:C:rotation-skip',
              'Skipped second Pflicht occupancy for rotation workplace (no eligible trainee)',
              {
                date: dateStr,
                workplace: targetWpC.name,
                timeslotId: targetTsIdC,
          ...
          let eligibleC = availableC.filter(
            (doc) =>
              !isExcluded(doc.id, targetWpC.id) &&
          ...
          if (
            eligibleC.length > 0 &&
            costFn.assignmentCost(eligibleC[0].id, targetWpC, dateStr, costContextC2) < Infinity
          ) {
      ...
      while (overChangedC) {
        ...
        const optionsC = nonAvailSlots
          ...
          .map((slot) => ({
            ...
            fillRatio:
              (phaseCSlotCount[slotKey(slot.wp.name, slot.timeslotId)] || 0) /
              Math.max(getOptimal(slot.wp), 1),
            deficit:
              Math.max(getOptimal(slot.wp), 1) -
              (phaseCSlotCount[slotKey(slot.wp.name, slot.timeslotId)] || 0),
          }))
        ...
        for (const docC of remainingC) {
          ...
          {
            const withPref = eligibleC3.filter(
              (o) => !hasOptionalQualReq(o.wp) || hasOptionalQuals(docC.id, o.wp.id),
            );
        ...
        if (bestAssignmentC && bestCostC < Infinity) {
          assignC(
            bestAssignmentC.doc.id,
            bestAssignmentC.wp.name,
            false,
            bestAssignmentC.timeslotId,
          );
      ...
      // ============================================================
      {
        ...
        if (phaseCSuggestions.length >= 2) {
            ...
            for (let i = 0; i < phaseCSuggestions.length && !swapImprovedC; i++) {
              for (let j = i + 1; j < phaseCSuggestions.length && !swapImprovedC; j++) {
                ...
                if (swappedCostC < currentCostC - 0.5) {
                  debugLog(
                    'phase:C:swap',
                    `Swap reduces cost by ${(currentCostC - swappedCostC).toFixed(1)}`,
                    {
                      before: `${doctorNameById[s1.doctor_id]}\u2192${s1.position}, ${doctorNameById[s2.doctor_id]}\u2192${s2.position}`,
                      after: `${doctorNameById[s2.doctor_id]}\u2192${s1.position}, ${doctorNameById[s1.doctor_id]}\u2192${s2.position}`,
          ...
          if (swapRoundsC > 1) {
            debugLog(
              'phase:C:swap-summary',
              `Swap optimization completed in ${swapRoundsC} rounds`,
              {
                date: dateStr,
  ...
  const finalResult = [...suggestions, ...autoFreiSuggestions] as SuggestionResult;
