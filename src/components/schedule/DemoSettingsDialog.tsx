...
interface DemoRowProps {
  rowName: string;
  setting: { active_days?: number[]; time?: string };
    ...
    <div className="flex flex-col space-y-3 p-4 border rounded-lg bg-slate-50">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Label htmlFor={`time-${rowName}`} className="text-xs text-slate-500">
            Uhrzeit:
          </Label>
          <Input
            id={`time-${rowName}`}
...
export default function DemoSettingsDialog() {
  ...
  const { data: settings = [], isLoading } = useQuery({
    ...
    queryFn: () => db.DemoSetting.list() as Promise<DemoSetting[]>,
  });
  ...
  const createOrUpdate = useMutation({
    mutationFn: async ({
      ...
      if (existing) {
        return db.DemoSetting.update(existing.id, { active_days, time });
      } else {
        return db.DemoSetting.create({ name, active_days, time });
      }
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['demoSettings'] }),
  });

  const getSetting = (name: string): { active_days: number[]; time: string } =>
    settings.find((s) => s.name === name) || { active_days: [1, 2, 3, 4, 5], time: '' };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Legen Sie fest, an welchen Tagen und zu welcher Uhrzeit die jeweiligen Demos
            stattfinden.
          </DialogDescription>
        </DialogHeader>
