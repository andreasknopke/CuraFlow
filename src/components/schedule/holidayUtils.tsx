...
interface SchoolHolidayRange {
  start: number;
  end: number;
  name: string;
}

interface SchoolRemovalRange {
  start: number;
  end: number;
}
...
export class HolidayCalculator {
  ...
  apiData: ApiHolidayData;
  publicHolidaysMap: Map<string, PublicHolidayEntry>;
  schoolHolidayRanges: SchoolHolidayRange[];
  schoolRemovals: SchoolRemovalRange[];

  constructor(
    stateCode: string = 'MV',
    customHolidays: CustomHoliday[] = [],
    apiData: ApiHolidayData = { school: [], public: [] },
  ) {
  ...
  init(): void {
    ...
    this.customHolidays
      .filter((h) => h.type === 'public' && h.action === 'add')
      .forEach((h) => {
    ...
    const publicRemovals = this.customHolidays.filter(
      (h) => h.type === 'public' && h.action === 'remove',
    );
    ...
    let schoolRanges: SchoolHolidayRange[] = [...(this.apiData.school || [])]
    ...
    this.customHolidays
      .filter((h) => h.type === 'school' && h.action === 'add')
      .forEach((h) => {
    ...
    const schoolRemovals: SchoolRemovalRange[] = this.customHolidays
      .filter((h) => h.type === 'school' && h.action === 'remove')
      .map((r) => ({
  ...
  isPublicHoliday(date: Date): PublicHolidayEntry | null {
  ...
  isSchoolHoliday(date: Date): { name: string } | null {
...
export const getEasterDate = getEaster;
