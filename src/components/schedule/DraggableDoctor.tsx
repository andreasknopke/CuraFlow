...
export default function DraggableDoctor({
  doctor,
  index,
  style,
  isDragDisabled,
  ...
  const targetWeekly = doctor.target_weekly_hours
    ? Number(doctor.target_weekly_hours)
    : workTimeModel
      ? Number(workTimeModel.hours_per_week)
      : doctor.fte && Number(doctor.fte) > 0
        ? Math.round(Number(doctor.fte) * DEFAULT_FULLTIME_HOURS * 10) / 10
        : null;
  ...
  return (
    <Draggable
      draggableId={`sidebar-doc-${doctor.id}`}
      index={index}
                ...
                <div className="flex-1 min-w-0 px-2 py-1.5">
                  ...
                  {targetWeekly !== null && (
                    <div className="flex items-center gap-1 text-[10px] leading-tight mt-0.5">
                      ...
                      {pct! > 100 && (
                        <span className="text-red-500" title="Überplanung!">
                          ⚠
                        </span>
                      )}
                    </div>
