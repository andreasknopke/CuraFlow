...
export default function MobileScheduleView({
  ...
  const [_viewTab, _setViewTab] = useState('day'); // 'day' | 'week'
  ...
  const groupedShifts = useMemo(() => {
    ...
    const other = dayShifts.filter(
      (s) =>
        !ABSENCE_POSITIONS.includes(s.position) &&
        !workplaces.find(
          (w) =>
            w.name === s.position &&
            ['Dienste', 'Rotationen', 'Demonstrationen & Konsile'].includes(w.category ?? ''),
        ),
    );
  ...
  const renderShiftCard = (shift: ShiftEntry, colorClass = 'bg-slate-100'): React.ReactNode => {
      ...
      <div className="bg-white border-b border-slate-200 p-4 sticky top-0 z-10">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            <div className="font-bold text-lg">{format(selectedDay, 'EEEE', { locale: de })}</div>
        ...
        {(isHoliday || isSchoolHol) && (
          <div
            className={`mt-2 text-xs py-1 px-2 rounded text-center ${isHoliday ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}
          >
