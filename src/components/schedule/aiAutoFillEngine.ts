...
interface AIAutoFillParams {
  ...
  _allDoctorQualifications?: unknown[];
...
function scorePlan(
  ...
  trainingRotations: TrainingRotation[],
  weekDayStrs: string[],
  qualData: QualData | undefined,
  existingShifts: ShiftEntry[],
): number {
  const {
    getDoctorQualIds = (() => []) as (id: number) => number[],
    getWpRequiredQualIds = (() => []) as (id: number) => number[],
...
function planToReadable(
  ...
  const { getDoctorQualIds, getWpRequiredQualIds, getWpExcludedQualIds, getWpDiscouragedQualIds } =
    qualData || {};
  ...
  for (const s of suggestions) {
    ...
    let label: string = nameMap[s.doctor_id] || String(s.doctor_id);
...
export async function generateAISuggestions(params: AIAutoFillParams): Promise<AIAutoFillResult> {
  const {
    weekDays,
    doctors,
    workplaces,
  ...
  console.log(
    `[AI AutoFill v3] Running ${NUM_VARIANTS} deterministic variants (cost-function scoring)...`,
  );
  ...
  console.log(
    `[AI AutoFill v3] Top 3: ${topVariants.map((v) => `#${v.index}(${v.score})`).join(', ')}`,
  );
  ...
  const doctorQuals: Record<number, number[]> = {};
  for (const doc of doctors) {
    doctorQuals[doc.id] = getDoctorQualIds(doc.id) || [];
  }
  const workplaceQuals: Record<
    number,
    { qualification_id: number; is_mandatory?: boolean; is_excluded?: boolean }[]
  > = {};
