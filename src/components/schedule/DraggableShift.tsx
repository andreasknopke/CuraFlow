...
interface DraggableShiftProps {
  ...
  fontSize?: number;
  boxSize?: number;
  currentUserDoctorId?: number | null;
  ...
  style?: React.CSSProperties & { backgroundColor?: string; color?: string };
  [key: string]: unknown;
}

const getDoctorShortLabel = (doctor: ShiftDoctor | null | undefined): string =>
  doctor?.initials || doctor?.name?.substring(0, 3) || '';

// Format TIME value (HH:MM:SS or HH:MM) to compact display (H:MM or HH:MM)
const formatShiftTime = (timeStr: string | null | undefined): string | null => {
...
const getTimeLabel = (shift: ShiftEntry): string | null => {
...
export default function DraggableShift({
  ...
  const timeslotBadgeClasses =
    timeslotLabelTone === 'warning' ? 'bg-amber-300 text-amber-950' : 'bg-indigo-600 text-white';
  ...
  if (isBeingDragged) {
    return (
      <Draggable
        draggableId={`${draggableIdPrefix}shift-${shift.id}`}
        index={index}
      ...
      {(provided, snapshot) => {
        ...
        const containerStyle = isDragging
          ...
          : {
              ...
              backgroundColor: props.style?.backgroundColor || '#f1f5f9',
              backgroundImage: wishMarker
                ? `linear-gradient(135deg, ${wishMarkerColor} 0, ${wishMarkerColor} 50%, transparent 50%, transparent 100%)`
                : undefined,
        ...
        return (
          <div
            ...
            title={
              combinedTooltip || (isPreview ? 'Vorschlag — per Drag & Drop verschieben' : undefined)
            }
          >
            {isDragging ? (
              ...
              <>
                ...
                {timeslotLabel && (
                  <span
                    ...
                    style={{
                      fontSize: `${Math.max(fontSize * 0.72, 9)}px`,
                      lineHeight: '1.4',
                      maxWidth: `${boxSize * 1.7}px`,
                      overflow: 'hidden',
                ...
                {fairnessInfo && (
                  <span
                    ...
                    style={{
                      fontSize: `${Math.max(fontSize * 0.65, 8)}px`,
                      backgroundColor:
                        fairnessInfo.total >= 4
                          ? '#ef4444'
                          : fairnessInfo.total >= 2
                            ? '#f59e0b'
                            : '#22c55e',
                      lineHeight: '1.4',
                    }}
                  ...
                  >
                    {fairnessInfo.total}D
                    {fairnessInfo.weekend > 0 ? ` ${fairnessInfo.weekend}W` : ''}
                    {fairnessInfo.wishText ? ' ★' : ''}
                  </span>
                )}
              </>
            ) : (
              ...
              <>
                ...
                <div className="flex flex-col items-center justify-center w-full relative z-10">
                  ...
                  {timeLabel && (
                    <span
                      ...
                      style={{
                        fontSize: `${Math.max(displayFontSize * 0.55, 7)}px`,
                        marginTop: '1px',
                      }}
                    >
                      {timeLabel}
                ...
                {fairnessInfo && (
                  <div
                    ...
                    style={{
                      fontSize: `${Math.max(fontSize * 0.55, 7)}px`,
                      backgroundColor:
                        fairnessInfo.total >= 4
                          ? '#ef4444'
                          : fairnessInfo.total >= 2
                ...
                {timeslotLabel && (
                  <div
                    ...
                    style={{
                      fontSize: `${Math.max(fontSize * 0.62, 8)}px`,
                      padding: '1px 4px',
                      maxWidth: `${boxSize + 12}px`,
                      overflow: 'hidden',
                      textOverflow: 'ellipsis',
                    }}
                    title={timeslotLabel}
