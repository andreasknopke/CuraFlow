...
export default function VoiceTrainingDialog({
  ...
  const { data: aliases = [], isLoading: _isLoading } = useQuery({
  ...
  const createAliasMutation = useMutation({
    mutationFn: (data: { doctor_id: number; detected_text: string }) => db.VoiceAlias.create(data),
    onSuccess: () => {
  ...
  const deleteAliasMutation = useMutation({
    mutationFn: (id: number) => db.VoiceAlias.delete(id),
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          const transcript = (await api.transcribeAudio(audioBlob)) as string;
          if (transcript) {
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Sprechen Sie den Namen so aus, wie Sie ihn normalerweise im Befehl verwenden
                    würden.
                  </p>
                </div>
                ...
                {detectedText && (
                  <div className="bg-white p-4 rounded-lg border shadow-sm space-y-3">
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <Button variant="outline" onClick={() => setDetectedText('')}>
                        Verwerfen
                      </Button>
                    </div>
                  </div>
                ...
                <div>
                  <div className="font-medium mb-2 text-sm text-slate-500">
                    Gespeicherte Aliases
                  </div>
                  <div className="space-y-2">
                    {aliases
                      .filter((a) => a.doctor_id === selectedDoctor.id)
                      .map((alias) => (
                        <div
                          key={alias.id}
