...
interface AssignmentContext {
  ...
  displacementCount?: Record<number, number>;
  rotationImpactScore?: Record<number, number>;
  serviceAssignedToday?: Set<number>;
  soleOccupantDoctors?: Set<number>;
  phase?: 'A' | 'B' | 'C';
}

interface CostFunctionConfig {
  doctors: Doctor[];
  workplaces: Workplace[];
  existingShifts: ShiftEntry[];
  suggestions: ShiftEntry[];
  ...
  wishes: Wish[];
  serviceHistory: Record<number, { fg: number; bg: number; weekend: number }>;
  ...
  getServiceType?: (name: string) => 'fg' | 'bg' | 'other';
  limitFG: number;
  limitBG: number;
  limitWeekend: number;
  isPublicHoliday: (date: Date) => unknown;
  autoFreiByDate: Record<string, Set<number>>;
...
export class CostFunction {
  ...
  _wpById: Record<number, Workplace>;
  _doctorById: Record<number, Doctor>;
  ...
  constructor({
    ...
    this.foregroundPositions =
      foregroundPositions || new Set(foregroundPosition ? [foregroundPosition] : []);
    this.backgroundPositions =
      backgroundPositions || new Set(backgroundPosition ? [backgroundPosition] : []);
  ...
  _qualificationCost(doctorId: number, workplace: Workplace): number {
  ...
  _getActiveRotationTargets(doctorId: number, dateStr: string): string[] {
    return this.trainingRotations
      .filter((r) => r.doctor_id === doctorId && r.start_date <= dateStr && r.end_date >= dateStr)
      .map((r) => {
        if (r.modality === 'Röntgen') {
          const roeWp = this.workplaces.find(
            (w) => w.name === 'DL/konv. Rö' || w.name.includes('Rö'),
          );
  ...
  _fairnessCost(doctorId: number, serviceName: string): number {
  ...
  _wishCost(doctorId: number, dateStr: string, positionName: string): number {
    // Hard block: approved "kein Dienst"
    const hasApprovedNoService = this.wishes.some(
      (w) =>
    ...
    const hasPendingNoService = this.wishes.some(
      (w) =>
        w.doctor_id === doctorId &&
        w.date === dateStr &&
        w.type === 'no_service' &&
    ...
    const serviceWish = this.wishes.find(
      (w) =>
        w.doctor_id === doctorId &&
        w.date === dateStr &&
        w.type === 'service' &&
    ...
    if (serviceWish) {
      if (!serviceWish.position || serviceWish.position === positionName) {
        return serviceWish.status === 'approved' ? WEIGHTS.WISH_APPROVED : WEIGHTS.WISH_PENDING;
      }
    }
  ...
  _limitCost(doctorId: number, serviceName: string, dateStr: string): number {
    const hist = this.serviceHistory[doctorId] || { fg: 0, bg: 0, weekend: 0 };
    const fte = this._doctorById[doctorId]?.fte ?? 1.0;
  ...
  _understaffingCost(
    ...
    const otherWps = this.workplaces.filter(
      (wp) =>
   ...
   */
  scorePlan(planSuggestions: ShiftEntry[], weekDayStrs: string[]): number {
    ...
    const variance =
      fteAdjusted.reduce((a, v) => a + (v - mean) ** 2, 0) / (fteAdjusted.length || 1);
    ...
    for (const rot of this.trainingRotations) {
      for (const dateStr of weekDayStrs) {
        if (rot.start_date <= dateStr && rot.end_date >= dateStr) {
          const assigned = planSuggestions.find(
            (s) =>
    ...
    score +=
      planSuggestions.filter(
        (s) => !absencePositions.includes(s.position) && s.position !== 'Verfügbar',
      ).length * 0.1;
    ...
    for (const dateStr of weekDayStrs) {
      ...
      for (const wp of this.workplaces) {
        ...
        const existing = this.existingShifts.filter(
          (s) => s.date === dateStr && s.position === wp.name,
        ).length;
