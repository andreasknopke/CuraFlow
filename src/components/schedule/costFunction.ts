...
export class CostFunction {
    ...
    constructor({
        ...
        this.getServiceType = getServiceType || ((name: string): string => {
    ...
    _getActiveRotationTargets(doctorId: string, dateStr: string): string[] {
    ...
    _fairnessCost(doctorId: string, serviceName: string): number {
    ...
    _wishCost(doctorId: string, dateStr: string, positionName: string): number {
    ...
    _limitCost(doctorId: string, serviceName: string, dateStr: string): number {
        const hist = this.serviceHistory[doctorId] || { fg: 0, bg: 0, weekend: 0 };
        const fte = this._doctorById[doctorId]?.fte ?? 1.0;
    ...
    _consecutiveCost(doctorId: string, workplace: any, dateStr: string, _context: any): number {
        ...
        const fmt = (dt: Date): string => `${dt.getFullYear()}-${pad(dt.getMonth()+1)}-${pad(dt.getDate())}`;
