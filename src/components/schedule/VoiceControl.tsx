...
export default function VoiceControl({
  ...
  // TODO: use proper Web Speech API types
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          setError('Transkriptionsfehler: ' + (err.response?.data?.error || err.message));
          setIsProcessing(false);
        } finally {
          // Stop tracks
  ...
  const toggleListening = () => {
    ...
    if (mode === 'transcribe') {
      ...
      if (!isWebSpeechSupported) {
        alert(
          'Ihr Browser unterstützt keine Spracherkennung. Bitte nutzen Sie Google Chrome oder wechseln Sie den Modus.',
        );
        return;
      }
  ...
  const handleSendText = async (text: string): Promise<void> => {
    ...
    try {
      ...
      const result = (await api.processVoiceCommand(text)) as VoiceCommandResult;
      ...
      const msg = e.response?.data?.error || e.message || 'Verarbeitungsfehler';
      setError(msg);
    } finally {
      setIsProcessing(false);
    }
  ...
  return (
    <div className="flex items-center gap-1 relative">
      ...
      {(isListening || isProcessing || transcript || error) && (
        <div
          className={`hidden md:block absolute top-full left-0 mt-2 text-xs px-3 py-1 rounded-lg z-50 whitespace-nowrap shadow-lg border transition-colors ${
            error
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 text-sm">
            <div className="text-slate-500 mb-2">
              Nutzt die Google-Engine im Browser oder ElevenLabs für High-Quality Erkennung.
