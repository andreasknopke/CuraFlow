...
export default function DatabaseManagement() {
  ...
  const [issues, setIssues] = useState<DatabaseIssue[] | null>(null);
  const [selectedIssues, setSelectedIssues] = useState<(string | number)[]>([]);

  // Wipe Database State
  const [showWipeDialog, setShowWipeDialog] = useState<boolean>(false);
  const [wipeConfirmText, setWipeConfirmText] = useState<string>('');
  const [isWiping, setIsWiping] = useState<boolean>(false);

  // --- CHECK ---
  const checkMutation = useMutation({
    mutationFn: (): Promise<InvokeResponse> => invokeWithAuth('check'),
    onSuccess: (res) => {
      setIssues(res.data.issues as DatabaseIssue[]);
      setSelectedIssues([]);
    },
  });
  ...
  const repairMutation = useMutation({
    mutationFn: async (): Promise<InvokeResponse> => {
      const issuesToFix = issues!.filter((i) =>
        selectedIssues.includes((i.id || i.ids?.[0]) as string | number),
      );
  ...
  const toggleAll = () => {
    ...
    else setSelectedIssues(issues!.map((i) => (i.id || i.ids?.[0]) as string | number));
  };
      ...
      <Dialog open={showWipeDialog} onOpenChange={setShowWipeDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Diese Aktion löscht <strong>alle Daten</strong> außer Benutzer (app_users). Dies kann
          ...
          <div className="py-4 space-y-4">
            <Alert className="bg-red-50 border-red-200">
              ...
              <AlertDescription className="text-red-700">
                Folgende Tabellen werden geleert: Doctor, ShiftEntry, Workplace, WishRequest,
                TrainingRotation, etc.
              </AlertDescription>
            </Alert>
            ...
            <Table>
              ...
              <TableBody>
                {issues.map((issue, idx) => {
                  const id = (issue.id || issue.ids?.[0]) as string | number;
                  return (
                    <TableRow key={idx}>
                      <TableCell>
                        <Checkbox
