...
export default function SystemLogs() {
  ...
  const [_selectedLog, _setSelectedLog] = useState<SystemLog | null>(null);
  ...
  const {
    ...
    queryKey: ['systemLogs'],
    queryFn: () => db.SystemLog.list({ sort: '-created_date', limit: 500 }) as Promise<SystemLog[]>,
                ...
                filteredLogs.map((log) => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`${getLevelBadge(log.level)} flex w-fit items-center gap-1`}
                      >
                        {getLevelIcon(log.level)}
                        <span className="capitalize">{log.level}</span>
                      </Badge>
                    </TableCell>
                    <TableCell className="font-medium text-slate-700">{log.source}</TableCell>
                    <TableCell>
