...
const TOKEN_KEY = DB_CREDENTIALS_KEY;
const TOKEN_ENABLED_KEY = DB_TOKEN_ENABLED_KEY;
const SAVED_TOKENS_KEY = SAVED_DB_TOKENS_KEY;
...
export const saveNamedToken = async (name: string, token: string): Promise<SavedToken> => {
  ...
  const existingIndex = tokens.findIndex((t: SavedToken) => t.name === name);
...
export const deleteNamedToken = async (id: string): Promise<void> => {
  const tokens = getSavedTokens().filter((t: SavedToken) => t.id !== id);
  localStorage.setItem(SAVED_TOKENS_KEY, JSON.stringify(tokens));
...
export const renameToken = async (id: string, newName: string): Promise<void> => {
  const tokens = getSavedTokens();
  const token = tokens.find((t: SavedToken) => t.id === id);
  if (token) {
    token.name = newName;
...
export const switchToToken = async (id: string): Promise<SavedToken> => {
  const tokens = getSavedTokens();
  const tokenEntry = tokens.find((t: SavedToken) => t.id === id);
...
export const syncSavedTokensFromIndexedDB = async (): Promise<SavedToken[]> => {
  try {
    ...
    const result = await new Promise<SavedToken[] | null>((resolve, reject) => {
      request.onsuccess = () => {
        db.close();
        resolve(request.result?.value || null);
      };
