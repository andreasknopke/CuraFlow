...
export default function QualificationOverview({
  ...
  const {
    qualifications,
    qualificationMap: _qualificationMap,
    isLoading: qualsLoading,
  } = useQualifications();
  const {
    allDoctorQualifications: _allDoctorQualifications,
    byDoctor,
    isLoading: dqLoading,
  } = useAllDoctorQualifications();

  const assignMutation = useMutation({
    mutationFn: (data: { doctor_id: string; qualification_id: string }) =>
      db.DoctorQualification.create(data),
    onSuccess: () => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-32 text-slate-400">Wird geladen...</div>
    );
  }
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center text-slate-500">
          ...
          <p className="text-sm">
            Verwenden Sie den <Shield className="w-4 h-4 inline" /> Qualifikations-Manager oben
            rechts, um Qualifikationen anzulegen.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-slate-500">
          ...
          {!isReadOnly &&
            ' Klicken Sie auf eine Zelle, um eine Qualifikation zuzuweisen oder zu entfernen.'}
        </p>
      </CardHeader>
      <CardContent>
                      ...
                      return (
                        ...
                        >
                          {hasQual ? (
                            ...
                            <span
                              className={`inline-block w-4 h-4 mx-auto rounded border ${
                                !isReadOnly
              ...
              <tr className="border-t bg-slate-50">
                ...
                {activeQuals.map((qual) => {
                  ...
                  return (
                    <td
                      key={qual.id}
                      className="text-center py-2 px-2 text-xs font-semibold text-slate-600"
                    >
