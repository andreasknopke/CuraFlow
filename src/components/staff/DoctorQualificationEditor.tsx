...
import type { Qualification, DoctorQualification } from '@/types/models';
...
export interface DoctorQualificationBadgesProps {
  doctorId: string;
 ...
 */
export default function DoctorQualificationEditor({
  doctorId,
  compact = false,
}: DoctorQualificationEditorProps) {
  ...
  const handleToggle = (qualId: string) => {
  ...
  if (activeQuals.length === 0) {
    return (
      <div className="text-xs text-slate-400 italic p-2">
        Noch keine Qualifikationen angelegt. Verwenden Sie den Qualifikations-Manager, um welche
        anzulegen.
      </div>
    );
  }
 ...
 */
export function DoctorQualificationBadges({
  doctorId,
  qualificationMap,
  allDoctorQualifications,
}: DoctorQualificationBadgesProps) {
