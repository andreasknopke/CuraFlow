...
const StaffingPlanInput = ({
  value: initialValue,
  onChange,
  disabled,
  className,
}: StaffingPlanInputProps) => {
...
export default function StaffingPlanTable({ doctors, isReadOnly }: StaffingPlanTableProps) {
  ...
  const { data: entries = [], isLoading: isLoadingEntries } = useQuery({
    ...
    queryFn: () => db.StaffingPlanEntry.filter({ year }) as Promise<StaffingPlanEntry[]>,
  });
  ...
  const updateEntryMutation = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      alert('Fehler beim Speichern: ' + (errObj.response?.data?.message || errObj.message));
  ...
  const getEntryValue = (doctorId: string, month: number): string => {
  ...
  const parseFTE = (val: string): number => {
    if (!val) return 0;
  ...
  const handleValueChange = (doctorId: string, month: number, newValue: string) => {
  ...
  const handleDialogSave = async () => {
    ...
    if (dialogInputType === 'number' && valueToSave) {
      ...
      if (!isNaN(parseFloat(normalized))) {
        const num = parseFloat(normalized);
        formattedValue = num.toLocaleString('de-DE', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        });
      }
  ...
  const getDoctorAverage = (doctorId: string): number => {
    let sum = 0;
    for (let m = 1; m <= 12; m++) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between bg-slate-50 p-4 rounded-lg border">
        ...
        <div className="flex items-center gap-4">
          <div className="text-sm text-slate-500">
            Legende: <span className="font-medium text-indigo-600">EZ</span> = Elternzeit,{' '}
            <span className="font-medium text-pink-600">MS</span> = Mutterschutz,{' '}
            <span className="font-medium text-red-600">KO</span> = Krank ohne Entgelt
          </div>
        </div>
      </div>
          ...
          <Table className="text-xs">
            <TableHeader>
              <TableRow className="bg-slate-100 hover:bg-slate-100">
                <TableHead className="w-[200px] font-bold text-slate-700 sticky left-0 bg-slate-100 z-20 border-r">
                  Name
                </TableHead>
                <TableHead className="text-center font-bold bg-slate-200 text-slate-900 border-r w-[60px]">
                  Ges.
                </TableHead>
                {Array.from({ length: 12 }).map((_, i) => (
                  <TableHead key={i} className="text-center font-semibold text-slate-600 w-[60px]">
                    {i + 1}
                  </TableHead>
                ))}
              </TableRow>
            ...
            <TableBody>
                  ...
                  <TableRow key={doc.id} className="hover:bg-slate-50">
                    <TableCell className="font-medium border-r sticky left-0 bg-white z-10">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-slate-400 bg-slate-100 px-1 rounded">
                          {doc.role?.substring(0, 2).toUpperCase()}
                        </span>
                      </div>
                    </TableCell>
                    ...
                    {Array.from({ length: 12 }).map((_, i) => {
                      ...
                      const entryExists = entries.some(
                        (e) => e.doctor_id === doc.id && e.month === month,
                      );
              ...
              <TableRow className="bg-slate-100 font-bold border-t-2 border-slate-200">
                <TableCell className="sticky left-0 bg-slate-100 border-r">
                  Gesamtergebnis
                </TableCell>
                <TableCell className="text-center border-r">
                  {formatNumber(yearlyAverageTotal)}
                </TableCell>
              ...
              <TableRow className="bg-slate-50 font-medium text-slate-600">
                <TableCell className="sticky left-0 bg-slate-50 border-r">
                  Stellenplan (Soll)
                </TableCell>
              ...
              <TableRow
                className={cn(
                  'font-bold border-t',
                  yearlyAverageTotal - targetFTE < 0
              ...
              >
                ...
                {monthlyTotals.map((total, i) => {
                  ...
                  return (
                    <TableCell
                      key={i}
                      className={cn('text-center', diff < 0 ? 'text-red-600' : 'text-green-600')}
                    >
                      {formatNumber(diff)}
                    </TableCell>
                  );
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Stellenplan bearbeiten</DialogTitle>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup
                value={dialogInputType}
                onValueChange={setDialogInputType}
                className="flex gap-4"
              >
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="number" id="input-number" />
                  <Label htmlFor="input-number" className="cursor-pointer">
                    Zahlenwert (FTE)
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="code" id="input-code" />
                  <Label htmlFor="input-code" className="cursor-pointer">
                    Statuscode
                  </Label>
                </div>
              </RadioGroup>
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={dialogCode} onValueChange={setDialogCode} className="grid gap-2">
                  {FTE_CODES.map((code) => (
                    <div
                      key={code}
                      className="flex items-center space-x-2 border rounded-lg p-3 hover:bg-slate-50"
                    >
            ...
            <div className="space-y-3 border-t pt-4">
              ...
              <RadioGroup
                value={dialogApplyMode}
                onValueChange={setDialogApplyMode}
