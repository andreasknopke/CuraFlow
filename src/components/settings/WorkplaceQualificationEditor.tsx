...
interface WorkplaceQualificationBadgesProps {
  workplaceId: string | number;
  qualificationMap?: Record<string | number, Qualification>;
  allWorkplaceQualifications?: Record<string | number, WorkplaceQualification[]>;
}
 ...
 */
export default function WorkplaceQualificationEditor({
  workplaceId,
}: WorkplaceQualificationEditorProps) {
  const queryClient = useQueryClient();
  const { qualifications, isLoading: qualsLoading } = useQualifications();

  const { data: workplaceQuals = [], isLoading: wqLoading } = useQuery<WorkplaceQualification[]>({
  ...
  const removeMutation = useMutation({
    mutationFn: (id: string | number) => db.WorkplaceQualification.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workplaceQualifications', workplaceId] });
      queryClient.invalidateQueries({ queryKey: ['allWorkplaceQualifications'] });
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string | number; data: Partial<WorkplaceQualification> }) =>
      db.WorkplaceQualification.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workplaceQualifications', workplaceId] });
      queryClient.invalidateQueries({ queryKey: ['allWorkplaceQualifications'] });
    },
  ...
  const _assignedQualIds = workplaceQuals.map((wq) => wq.qualification_id);

  const handleToggle = (qualId: string | number) => {
  ...
  const cycleMode = (wqEntry: WorkplaceQualification) => {
  ...
  const getModeLabel = (wqEntry: WorkplaceQualification): string => {
  ...
  const getModeStyle = (wqEntry: WorkplaceQualification): string => {
    if (wqEntry.is_mandatory && wqEntry.is_excluded)
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-xs text-slate-500 mb-1">
        Pflicht = Mitarbeiter muss Qualifikation besitzen. Sollte = Bevorzugt qualifiziert, aber
        Unqualifizierte erlaubt. Sollte nicht = Qualifizierte nur wenn kein anderer verfügbar. Nicht
        = Mitarbeiter mit dieser Qualifikation darf hier nicht eingeteilt werden.
      </div>
      <div className="space-y-1">
        {activeQuals.map((qual) => {
          ...
          return (
            ...
            >
              <Checkbox checked={isAssigned} onCheckedChange={() => handleToggle(qual.id)} />
              <Badge
                style={{
 ...
 */
export function WorkplaceQualificationBadges({
  workplaceId,
  qualificationMap,
  allWorkplaceQualifications,
}: WorkplaceQualificationBadgesProps) {
