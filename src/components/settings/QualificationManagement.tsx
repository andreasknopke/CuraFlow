...
import { Award, Plus, Trash2, Pencil, Shield, ChevronDown, ChevronUp } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
// Kategorie-Optionen
const CATEGORY_OPTIONS = ['Medizinisch', 'Dienst', 'Zertifizierung', 'Gerät', 'Sonstiges'];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Qualifikationen können Mitarbeitern zugeordnet und als Voraussetzung für Arbeitsplätze
            definiert werden.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, short_label: e.target.value.substring(0, 5) })
                }
                placeholder="z.B. CT"
                maxLength={5}
              />
          ...
          <div className="grid gap-2">
              ...
              {COLOR_PRESETS.map((preset) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                    formData.color_bg === preset.bg ? 'ring-2 ring-offset-1 ring-indigo-500' : ''
                  }`}
            ...
            <div className="mt-2 flex items-center gap-3">
              ...
              <Badge
                ...
                className="border-0"
              >
                {formData.short_label || formData.name.substring(0, 3).toUpperCase()} –{' '}
                {formData.name || 'Name'}
              </Badge>
            </div>
          </div>
...
export default function QualificationManagement() {
  ...
  const handleSave = (formData: QualFormData) => {
    if (editingQual) {
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Definieren Sie Qualifikationen und Berechtigungen, die Mitarbeitern zugeordnet und für
              Arbeitsplätze als Voraussetzung definiert werden können.
            </DialogDescription>
          </DialogHeader>
                    ...
                    {expandedCategories[cat] !== false && (
                      <div className="space-y-2 mt-2 ml-2">
                        {(qualificationsByCategory[cat] || []).map((qual) => (
                          <Card
                            key={qual.id}
                            className={qual.is_active === false ? 'opacity-50' : ''}
                          >
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Die Qualifikation „{qual.name}" wird gelöscht. Alle
                                        Zuordnungen zu Mitarbeitern und Arbeitsplätzen werden
                                        ebenfalls entfernt.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
