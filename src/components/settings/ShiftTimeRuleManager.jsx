...
export default function ShiftTimeRuleManager({ isReadOnly = false }) {
  ...
  const saveModel = async () => {
    ...
    if (!code) { toast.error('Bitte ein Kürzel eingeben.'); return; }
    if (!modelForm.workplace_id) { toast.error('Bitte einen Arbeitsplatz auswählen.'); return; }

    const isDuplicate = existingCodes.some(c =>
      c === code.toLowerCase() && (addingModel || code.toLowerCase() !== editingModel?.toLowerCase())
    );
    if (isDuplicate) { toast.error(`Das Kürzel "${code}" ist bereits vergeben.`); return; }

    if (addingModel) {
      setAddingModel(false);
      setPendingModel({ short_code: code, label: modelForm.label.trim(), workplace_id: modelForm.workplace_id });
      setExpandedModels(prev => new Set([...prev, code]));
      setAddingTimeFor(code);
      setTimeForm({ work_time_model_id: '', start_time: '07:00', end_time: '15:12', break_minutes: 0 });
  ...
  const cardProps = (group, isPending) => ({
    ...
    onStartAddTime: startAddTime, onStartEditTime: startEditTime,
    onSaveTime: saveTime, onDeleteTime: deleteTime, onResetTimeForm: resetTimeForm,
    availableModels: availableModelsFor(group.code),
    formatTime, calcHours,
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold flex items-center gap-2"><Clock size={18} />Dienstmodelle</h3>
          <p className="text-sm text-slate-500 mt-0.5">
            Jedes Dienstmodell hat ein Kürzel, eine Beschreibung und pro Arbeitszeitmodell eigene Zeiten.
          </p>
        </div>
        {!isReadOnly && !addingModel && (
          <Button size="sm" onClick={startAddModel}><Plus size={14} className="mr-1" />Neues Dienstmodell</Button>
        )}
      </div>
          ...
          <CardContent className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
              ...
              <div className="flex gap-2">
                <Button size="sm" className="h-9" onClick={saveModel}><Plus size={14} className="mr-1" />Anlegen</Button>
                <Button size="sm" variant="ghost" className="h-9" onClick={resetModelForm}><X size={14} /></Button>
              </div>
            </div>
          </CardContent>
        </Card>
...
function ModelCard({
  group, isExpanded, onToggle, workplaceMap, modelMap, workTimeModels,
  isReadOnly, editingModel, modelForm, setModelForm,
  onEditModel, onSaveModel, onCancelEditModel, onDeleteModel,
  addingTimeFor, editingTimeId, timeForm, setTimeForm,
      ...
      <CardHeader className="py-3 px-4">
        {isEditingHeader ? (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            ...
            <div className="flex gap-2">
              <Button size="sm" className="h-9" onClick={onSaveModel}><Save size={14} className="mr-1" />Speichern</Button>
              <Button size="sm" variant="ghost" className="h-9" onClick={onCancelEditModel}><X size={14} /></Button>
            </div>
          </div>
        ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-400">
                  {wp?.name || group.workplace_id} · {group.entries.length} {group.entries.length === 1 ? 'Zeiteintrag' : 'Zeiteinträge'}
                </div>
              </div>
            </div>
            {!isReadOnly && (
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-50 border-y border-slate-200">
                <th className="text-left py-1.5 px-3 font-medium text-slate-600 w-[200px]">Arbeitszeitmodell</th>
              ...
              {group.entries.map(entry => {
                    ...
                    <tr key={entry.id} className="border-b border-slate-200 bg-indigo-50/50">
                      ...
                      <td className="py-1.5 px-3">
                        <div className="flex gap-1">
                          <Button size="sm" className="h-7 px-2 text-xs" onClick={() => onSaveTime(group.code)}><Save size={11} className="mr-0.5" />OK</Button>
                          <Button size="sm" variant="ghost" className="h-7 px-1" onClick={onResetTimeForm}><X size={12} /></Button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                return (
                  <tr key={entry.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                    <td className="py-1.5 px-3 text-slate-700">{m ? `${m.name} (${m.hours_per_week}h)` : entry.work_time_model_id}</td>
                    <td className="py-1.5 px-3 font-mono text-slate-700">{formatTime(entry.start_time)}</td>
                    <td className="py-1.5 px-3 font-mono text-slate-700">{formatTime(entry.end_time)}</td>
