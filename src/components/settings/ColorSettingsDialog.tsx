 ...
 */
export function getDefaultRotationColor(index: number): ColorPair {
...
export default function ColorSettingsDialog() {
  ...
  const createOrUpdateMutation = useMutation({
    mutationFn: async (data: {
      ...
      const existing = colorSettings.find(
        (s) => s.name === data.name && s.category === data.category,
      );
      if (existing) {
  ...
  const getColor = (name: string, category: string, index?: number): ColorPair => {
    ...
    if (category === 'section')
      return (
        (DEFAULT_COLORS.sections as Record<string, ColorPair>)[name] || {
          bg: '#e2e8f0',
          text: '#1e293b',
        }
      );
    if (category === 'role')
      return (
        (DEFAULT_COLORS.roles as Record<string, ColorPair>)[name] || {
          bg: '#f3f4f6',
          text: '#1f2937',
        }
      );
    if (category === 'position')
      return (
        (DEFAULT_COLORS.positions as Record<string, ColorPair>)[name] || {
          bg: '#f1f5f9',
          text: '#1e293b',
        }
      );
    ...
    if (category === 'absence')
      return (
        (DEFAULT_COLORS.absences as Record<string, ColorPair>)[name] || {
          bg: '#64748b',
          text: '#ffffff',
        }
      );
  ...
  const rotationsList = workplaces
          ...
          <TabsContent value="rotations" className="space-y-4 mt-4">
            {rotationsList.length === 0 ? (
              <p className="text-sm text-slate-500 italic">
                Keine Rotationen konfiguriert. Bitte fügen Sie unter "Arbeitsplätze" Einträge in der
                Kategorie "Rotationen" hinzu.
              </p>
            ) : (
