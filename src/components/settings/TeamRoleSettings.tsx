...
import { Users, Plus, Trash2, GripVertical, Pencil, Settings2 } from 'lucide-react';
...
export function useTeamRoles() {
  const {
    ...
    queryKey: ['teamRoles'],
    queryFn: () => db.TeamRole.list() as Promise<TeamRole[]>,
  ...
  const rolePriority = teamRoles.reduce<Record<string, number>>((acc, role, idx) => {
  ...
  const foregroundDutyRoles = teamRoles
    .filter((r) => r.can_do_foreground_duty !== false)
    .map((r) => r.name);
  // Fallback: Wenn can_do_background_duty undefined ist, auf is_specialist zurückfallen (altes Verhalten)
  const backgroundDutyRoles = teamRoles
    .filter(
      (r) =>
        r.can_do_background_duty === true ||
        (r.can_do_background_duty === undefined && r.is_specialist),
    )
    .map((r) => r.name);
  // Fallback: Wenn excluded_from_statistics undefined ist, prüfe auf Nicht-Radiologe (altes Verhalten)
  const statisticsExcludedRoles = teamRoles
    .filter(
      (r) =>
        r.excluded_from_statistics === true ||
        (r.excluded_from_statistics === undefined && r.name === 'Nicht-Radiologe'),
    )
    .map((r) => r.name);

  // Helper-Funktion um Berechtigungen zu prüfen (mit Fallback für alte DBs)
  const canDoForegroundDuty = (roleName: string) => {
    const role = teamRoles.find((r) => r.name === roleName);
    return role ? role.can_do_foreground_duty !== false : true;
  };

  const canDoBackgroundDuty = (roleName: string) => {
    const role = teamRoles.find((r) => r.name === roleName);
  ...
  const isExcludedFromStatistics = (roleName: string) => {
    const role = teamRoles.find((r) => r.name === roleName);
    if (!role) return false;
...
export async function initializeDefaultRoles(): Promise<TeamRole[]> {
  try {
    const existing = (await db.TeamRole.list()) as TeamRole[];
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          <DialogTitle>{role ? 'Funktion bearbeiten' : 'Neue Funktion hinzufügen'}</DialogTitle>
          <DialogDescription>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, can_do_foreground_duty: e.target.checked })
                  }
                  className="h-4 w-4 rounded border-slate-300"
                />
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, can_do_background_duty: e.target.checked })
                  }
                  className="h-4 w-4 rounded border-slate-300"
                />
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, excluded_from_statistics: e.target.checked })
                  }
                  className="h-4 w-4 rounded border-slate-300"
                />
...
export default function TeamRoleSettings() {
  ...
  const { data: teamRoles = [], isLoading: _isLoading } = useQuery({
    queryKey: ['teamRoles'],
    queryFn: async () => {
      const roles = (await db.TeamRole.list()) as TeamRole[];
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string | number; data: Partial<TeamRole> }) =>
      db.TeamRole.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['teamRoles'] });
      setEditDialogOpen(false);
      setEditingRole(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string | number) => db.TeamRole.delete(id),
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Definieren Sie die Funktionen/Rollen für Ihr Team. Die Reihenfolge bestimmt die
          ...
          <div className="py-4">
                  ...
                  <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-2">
                    {teamRoles.map((role, index) => (
                      <Draggable
                        ...
                        draggableId={String(role.id || role.name)}
                        index={index}
                      >
                        {(provided, snapshot) => (
                          <div
                          ...
                          >
                            <Card
                              className={`${snapshot.isDragging ? 'shadow-lg ring-2 ring-indigo-500' : ''}`}
                            >
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                          Die Funktion "{role.name}" wird gelöscht. Bestehende
                                          Teammitglieder mit dieser Funktion behalten ihre
                                          Zuordnung.
                                        </AlertDialogDescription>
                                      </AlertDialogHeader>
                                      <AlertDialogFooter>
            ...
            <Button onClick={handleAddNew} variant="outline" className="w-full mt-4">
