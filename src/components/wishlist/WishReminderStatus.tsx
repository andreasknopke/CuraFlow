 ...
 */
export default function WishReminderStatus({
  targetMonth,
  compact = false,
}: WishReminderStatusProps) {
  const { data, isLoading, error } = useQuery<ReminderStatusData>({
    queryKey: ['wish-reminder-status', targetMonth],
    queryFn: async () => {
      const res = await (api as unknown as { fetch: (url: string) => Promise<Response> }).fetch(
        `/api/staff/wish-reminder-status?month=${targetMonth}`,
      );
                ...
                return (
                  <Tooltip key={d.doctor_id}>
                    <TooltipTrigger asChild>
                      <div
                        className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${cfg.color} cursor-default`}
                      >
