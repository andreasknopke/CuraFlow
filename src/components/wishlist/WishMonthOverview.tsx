...
interface WishMonthOverviewProps {
  ...
  onToggle?: (date: Date, wish: WishEntry | null, doctorId: number, e: React.MouseEvent) => void;
...
export default function WishMonthOverview({
  ...
  const { user } = useAuth();
  const [hiddenDoctorIds, setHiddenDoctorIds] = useState<number[]>([]);
  ...
  useEffect(() => {
    ...
    if (Array.isArray(user.wish_overview_hidden_doctors)) {
      setHiddenDoctorIds(user.wish_overview_hidden_doctors as number[]);
    }
  }, [user]);
  ...
  const saveHiddenDoctors = async (newHiddenIds: number[]) => {
    try {
  ...
  const hasAnyWish = (date: Date) => {
    const dateStr = format(date, 'yyyy-MM-dd');
    return wishes.some(
      (w) => w.date === dateStr && w.type === 'service' && w.position === activeType,
    );
  };
  ...
  const toggleDoctorVisibility = (docId: number) => {
        ...
        <TooltipProvider>
          <Tooltip delayDuration={0}>
            <TooltipTrigger asChild>
              <div
                className={`w-full h-full min-h-[40px] flex items-center justify-center border border-transparent rounded-sm ${bgColor} ${textColor} text-[10px] font-bold cursor-not-allowed`}
              >
                {shortLabel}
              </div>
            </TooltipTrigger>
    ...
    if (!wish || wish.status === 'rejected') {
      ...
      return (
        <div
          ...
          onClick={(e) => onToggle?.(date, null, doctor.id, e)}
        ></div>
      );
    }
    ...
    if (wish.type === 'service') {
      bgColor =
        wish.status === 'approved'
          ...
          : wish.status === 'rejected'
            ? 'bg-green-50 opacity-50 grayscale'
            : 'bg-green-100';
      ...
      bgColor =
        wish.status === 'approved'
          ? 'bg-red-200'
          : wish.status === 'rejected'
            ? 'bg-red-50 opacity-50 grayscale'
            : 'bg-red-100';
        ...
        <Tooltip delayDuration={0}>
          <TooltipTrigger asChild>
            <div
              ...
              onClick={(e) => onToggle?.(date, wish, doctor.id, e)}
            >
              {icon}
              {wish.status === 'pending' && (
          ...
          <TooltipContent side="top">
            <div className="text-xs">
              ...
              <div className="my-1 border-t border-white/20 pt-1">
                {wish.type === 'service' ? (
                  <span className="text-green-300 font-bold">
                    Dienstwunsch: {wish.position || 'Allgemein'}
                  </span>
                ) : (
                  <span className="text-red-300 font-bold">Kein Dienst</span>
                )}
              </div>
              ...
              {wish.admin_comment && (
                <div className="mt-1 text-amber-300 border-t border-white/10 pt-1">
            ...
            {days.map((day) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      className={`font-bold ${isWknd || isHol ? 'text-red-500' : isSchoolHol ? 'text-green-700' : 'text-slate-700'} ${isToday ? 'text-blue-600' : ''}`}
                    >
                      {format(day, 'dd.MM.')}
                    </span>
                  ...
                  {visibleDoctors.map((doc) => (
                    <div
                      ...
                      className="w-[45px] flex-shrink-0 p-0.5 border-r border-slate-100 last:border-r-0 relative group"
                    >
                      {renderCell(doc, day)}
                    </div>
                  ))}
            ...
            <div className="grid gap-2">
              ...
              >
                ...
                <span className="font-medium">
                  {areAllDoctorsVisible ? 'Keinen ansehen' : 'Alle ansehen'}
                </span>
              </div>
              {doctors.map((doc) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      <span
                        className={`font-medium ${isHidden ? 'text-slate-400' : 'text-slate-900'}`}
                      >
                        {doc.name}
                      </span>
