...
interface VacationOverviewCellProps {
  ...
  visibleTypes: string[];
  customColors: Record<string, { backgroundColor: string; color: string }>;
  dragInfo: DragInfo;
...
interface VacationOverviewProps {
  ...
  isReadOnly?: boolean;
  monthsPerRow?: number;
  minPresentSpecialists?: number;
  minPresentAssistants?: number;
}

// Memoized Cell Component
const VacationOverviewCell = memo<VacationOverviewCellProps>(
  ({
    date,
    doctor,
    status,
...
export default function VacationOverview({
  ...
  isReadOnly,
  monthsPerRow = 3,
  minPresentSpecialists = 2,
  minPresentAssistants = 4,
  ...
  const handleToggle = React.useCallback(
    (date: Date, status: string | null, docId: string, e: React.MouseEvent) => {
  ...
  const dailyAbsences = React.useMemo(() => {
    ...
    shifts.forEach((s) => {
      ...
      if (!['Urlaub', 'Krank', 'Frei', 'Dienstreise', 'Nicht verfügbar'].includes(s.position))
        return;
      ...
      } else if (specialistRoles.includes(doc.role as string)) {
        entry.specialists++;
  ...
  const totalStaff = React.useMemo(() => {
    ...
    doctors.forEach((d) => {
      ...
      else if (specialistRoles.includes(d.role as string)) specialists++;
    });
  ...
  const vacationCounts = React.useMemo(() => {
    ...
    shifts.forEach((s) => {
      if (s.position === 'Urlaub') {
        ...
        if (!isWeekend(d) && !(isPublicHoliday && isPublicHoliday(d))) {
          if (s.doctor_id != null && counts[s.doctor_id as string] !== undefined) {
            counts[s.doctor_id as string]++;
          }
        }
      }
    });
            ...
            <table className="w-full border-collapse text-xs table-fixed">
              <thead>
                ...
                <tr>
                  ...
                  <th
                    className="sticky left-[150px] z-20 bg-slate-100 border-b border-r p-2 w-[50px] min-w-[50px] text-center shadow-[1px_0_0_0_rgba(0,0,0,0.1)]"
                    title="Verplante Urlaubstage (Netto)"
                  >
                    Urlaub
                  </th>
                  {months.map((m) => {
                    ...
                    return (
                      <th
                        key={m}
                        colSpan={daysInMonth}
                        className="border-b border-r bg-slate-50 p-1 text-center font-bold text-slate-700"
                      >
                ...
                <tr>
                  ...
                  <th className="sticky left-[150px] z-20 bg-slate-100 border-b border-r p-1 text-center text-[10px] text-slate-500 shadow-[1px_0_0_0_rgba(0,0,0,0.1)]">
                    ∑
                  </th>
                  {months.map((m) => {
                    const date = setMonth(setYear(new Date(), year), m);
                    ...
                    return Array.from({ length: daysInMonth }).map((_, i) => {
                            ...
                            <Popover>
                              <PopoverTrigger asChild>
                                <div className="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 z-30 cursor-pointer">
                                  <AlertTriangle
                                    className="w-3 h-3 text-red-600 bg-white rounded-full shadow-sm border border-red-200"
                                    fill="currentColor"
                                    fillOpacity={0.2}
                                  />
                                </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="text-xs space-y-2">
                                    {specsLow && (
                                      <div>
                                        <div className="font-semibold text-slate-700">
                                          Verfügbare Fachärzte: {presentSpecialists} (Min:{' '}
                                          {minPresentSpecialists})
                                        </div>
                                        <div className="text-slate-500 mb-1">Abwesend:</div>
                                        <ul className="list-disc list-inside text-slate-500 ml-1">
                                          {absences.specialistDetails.map((n: string) => (
                                            <li key={n}>{n}</li>
                                          ))}
                                        </ul>
                                      </div>
                                    ...
                                    {asstsLow && (
                                      <div>
                                        <div className="font-semibold text-slate-700">
                                          Verfügbare Ass.-Ärzte: {presentAssistants} (Min:{' '}
                                          {minPresentAssistants})
                                        </div>
                                        <div className="text-slate-500 mb-1">Abwesend:</div>
                                        <ul className="list-disc list-inside text-slate-500 ml-1">
                                          {absences.assistantDetails.map((n: string) => (
                                            <li key={n}>{n}</li>
                                          ))}
                                        </ul>
                                      </div>
                      ...
                      return (
                        <th
                          key={`${m}-${i}`}
                          className={`relative border-b border-r p-0.5 text-[10px] text-center w-[22px] min-w-[22px] ${headerClass}`}
                        >
                          {i + 1}
                          {warning}
                        </th>
