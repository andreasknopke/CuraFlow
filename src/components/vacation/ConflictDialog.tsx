...
export default function ConflictDialog({
  ...
  const totalToDelete =
    deleteConflicts.length + (keepOptionalServices ? 0 : optionalConflicts.length);
  ...
  const formatDateRange = (dates: Conflict[]) => {
    if (dates.length === 0) return '';
    if (dates.length === 1) return format(new Date(dates[0].date), 'dd.MM.yyyy', { locale: de });
    if (dates.length <= 3)
      return dates.map((d) => format(new Date(d.date), 'dd.MM.', { locale: de })).join(', ');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Für <strong>{doctorName}</strong> gibt es bestehende Einträge, die von dieser Änderung
            betroffen sind.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              ...
              <div className="flex items-center space-x-2 pt-2 border-t border-blue-200">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setKeepOptionalServices(!!checked)}
                />
                <label
        ...
        <div className="bg-slate-100 rounded-lg p-3 text-sm">
          ...
          <ul className="mt-1 text-slate-600">
            ...
            {totalToOverwrite > 0 && (
              <li>• {totalToOverwrite} Abwesenheit(en) werden überschrieben</li>
            )}
