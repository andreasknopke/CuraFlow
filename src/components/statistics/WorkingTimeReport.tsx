...
import type { Doctor, ShiftEntry, Workplace, WorkplaceTimeslot } from '@/types';

interface DoctorWorkStats {
    doctor: Doctor;
    totalMinutes: number;
    totalHours: string;
    totalDays: number;
    avgHoursPerDay: string;
    dailyDetails: Record<string, { minutes: number; hours: string; shifts: number }>;
}
 ...
 */
function mergeTimeIntervals(intervals: { start: number; end: number }[]): number {
 ...
 */
function shiftToInterval(shift: ShiftEntry, timeslot: WorkplaceTimeslot | null, workplace: Workplace | null): { start: number; end: number; rawDuration: number; adjustedDuration: number } {
...
export default function WorkingTimeReport() {
    ...
    const [viewMode, setViewMode] = useState<"day" | "week" | "month" | "year">("month"); // day, week, month, year
