...
export default function ComplianceReport({ doctors, shifts }: ComplianceReportProps) {
  const stats = useMemo<ComplianceStat[]>(() => {
    return doctors
      .map((doc) => {
        ...
        const weekendShifts = docShifts.filter((s) => {
          ...
          return (
            isValid(d) &&
            isWeekend(d) &&
            !['Frei', 'Urlaub', 'Krank', 'Dienstreise', 'Nicht verfügbar'].includes(s.position)
          );
        }).length;
                ...
                .map((doc) => (
                  <div key={doc.name} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-sm font-bold ${doc.maxStreak > 10 ? 'text-red-600' : 'text-slate-700'}`}
                      >
                        {doc.maxStreak} Tage
                      </span>
                    </div>
