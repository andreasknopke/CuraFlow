...
interface StatisticsData {
    ...
    byDoctor: StatisticsDoctor[];
}

function buildFileSuffix(month: string): string {
    if (month === 'all') {
        return 'gesamt';
    }
...
function buildStatisticsMatrix(stats: StatisticsData): { headers: string[]; rows: (string | number)[][] } {
    const headers = [
        'Name',
        'Rolle',
        'Gesamt Dienste',
...
export function exportStatisticsCsv({ stats, year, month }: { stats: StatisticsData; year: number | string; month: string }): void {
...
export function exportStatisticsExcel({ stats, year, month, title }: { stats: StatisticsData; year: number | string; month: string; title: string }): void {
    const { headers, rows } = buildStatisticsMatrix(stats);
...
export function exportStatisticsPdf({ stats, year, month, title }: { stats: StatisticsData; year: number | string; month: string; title: string }): void {
    const { headers, rows } = buildStatisticsMatrix(stats);
