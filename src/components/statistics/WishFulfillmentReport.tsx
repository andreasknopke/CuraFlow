...
interface WishFulfillmentReportProps {
  doctors: Doctor[];
  wishes: WishEntry[];
  shifts: ShiftEntry[];
}
...
export default function WishFulfillmentReport({
  doctors,
  wishes,
  shifts,
}: WishFulfillmentReportProps) {
  const stats = useMemo<WishStat[]>(() => {
    const data = doctors
      .map((doc) => {
        ...
        docWishes.forEach((wish) => {
          ...
          if (wish.type === 'service') {
            ...
            const isServiceShift =
              shiftOnDate &&
              (shiftOnDate.position.includes('Dienst') || shiftOnDate.position === 'Spätdienst');
      ...
      .filter((item) => item !== null) as WishStat[];
