...
import type { Doctor, ShiftEntry, Workplace, StaffingPlanEntry } from '@/types/models';
...
export default function TransferToSchedulerDialog({
  ...
  const transferPreview = useMemo(() => {
    ...
    rotations.forEach((rot) => {
      ...
      days.forEach((day) => {
        ...
        const activeDaysRaw = wp?.active_days as number[] | undefined;
        const activeDays =
          activeDaysRaw && activeDaysRaw.length > 0 ? activeDaysRaw : [1, 2, 3, 4, 5];
        ...
        const available = isDoctorAvailable(
          doctor as unknown as Parameters<typeof isDoctorAvailable>[0],
          day,
          staffingPlanEntries as unknown as Parameters<typeof isDoctorAvailable>[2],
        );
        if (!available) {
          skipped.push({
            date: dateStr,
    ...
    entries.sort(
      (a, b) => a.date.localeCompare(b.date) || a.doctorName.localeCompare(b.doctorName),
    );
    skipped.sort(
      (a, b) => a.date.localeCompare(b.date) || a.doctorName.localeCompare(b.doctorName),
    );
  ...
  const doctorSummary = useMemo(() => {
    ...
    transferPreview.entries.forEach((e) => {
      const name = e.doctorName as string;
      const modality = e.modality as string;
      if (!summary[name]) {
        summary[name] = { count: 0, modalities: new Set() };
      }
      summary[name].count++;
      summary[name].modalities.add(modality);
    });
    return summary;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Übertragen Sie die geplanten Ausbildungsrotationen als Schichteinträge in den
            Wochenplan.
          </DialogDescription>
        </DialogHeader>
        ...
        {!showPreview ? (
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup
                value={transferMode}
                onValueChange={(val) => setTransferMode(val as 'day' | 'week' | 'from_date')}
                className="space-y-2"
              >
                <div className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-slate-50 transition-colors">
                  ...
                  <Label htmlFor="mode-day" className="flex-1 cursor-pointer">
                    ...
                    <div className="text-sm text-slate-500">
                      Nur den ausgewählten Tag übertragen
                    </div>
                  </Label>
                </div>
                <div className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-slate-50 transition-colors">
                  ...
                  <Label htmlFor="mode-week" className="flex-1 cursor-pointer">
                    ...
                    <div className="text-sm text-slate-500">
                      Die gesamte Woche des ausgewählten Datums
                    </div>
                  </Label>
                </div>
                <div className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-slate-50 transition-colors">
                  ...
                  <Label htmlFor="mode-from" className="flex-1 cursor-pointer">
                    ...
                    <div className="text-sm text-slate-500">
                      Alle Rotationen ab dem ausgewählten Datum bis Jahresende
                    </div>
                  </Label>
                </div>
              </RadioGroup>
            ...
            <div className="space-y-2">
              ...
              {transferMode === 'week' && (
                <p className="text-sm text-slate-500">
                  Woche: {format(dateRange.start, 'dd.MM.yyyy')} –{' '}
                  {format(dateRange.end, 'dd.MM.yyyy')}
                </p>
              )}
              {transferMode === 'from_date' && (
                <p className="text-sm text-slate-500">
                  Zeitraum: {format(dateRange.start, 'dd.MM.yyyy')} –{' '}
                  {format(dateRange.end, 'dd.MM.yyyy')}
                </p>
              )}
            </div>
            ...
            <div className="flex items-start space-x-3 p-4 rounded-lg border bg-amber-50 border-amber-200">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700 mt-1">
                  Wenn aktiviert, werden bestehende Dienst- und Rotationseinträge durch die
                  Ausbildungsrotation ersetzt. Abwesenheiten (Urlaub, Krank, etc.) werden niemals
                  überschrieben.
                </p>
              </div>
            </div>
              ...
              <div className="flex gap-2">
                ...
                <div>
                  ...
                  <ul className="mt-1 space-y-1 list-disc list-inside">
                    ...
                    <li>
                      Abwesende Mitarbeiter (Urlaub, Krank, etc.) werden automatisch übersprungen
                    </li>
          ...
          <div className="flex-1 overflow-hidden flex flex-col gap-4 py-2">
            ...
            <div className="grid grid-cols-3 gap-4 p-4 bg-slate-50 rounded-lg">
              <div className="text-center">
                <div className="text-2xl font-bold text-emerald-600">
                  {transferPreview.entries.length}
                </div>
                <div className="text-sm text-slate-600">Einträge erstellen</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-amber-600">
                  {transferPreview.entries.filter((e) => e.overwrite).length}
                </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {transferPreview.entries.map((entry, idx) => (
                      <TableRow
                        key={idx}
                        className={entry.overwrite ? 'bg-amber-50' : 'bg-emerald-50'}
                      >
                        <TableCell>
                          {entry.overwrite ? (
                        ...
                        <TableCell>
                          {format(new Date(entry.date), 'EE, dd.MM.yyyy', { locale: de })}
                        </TableCell>
                        <TableCell>
                      ...
                      {transferPreview.skipped.map((item, idx) => (
                        <TableRow key={idx} className="bg-slate-50">
                          ...
                          <TableCell className="text-xs">
                            {format(new Date(item.date), 'EE, dd.MM.', { locale: de })}
                          </TableCell>
            ...
            {Object.keys(doctorSummary).length > 0 && (
              <div className="p-3 bg-emerald-50 rounded-lg">
                <h4 className="font-medium mb-2 text-emerald-800 text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(doctorSummary)
                    ...
                    .map(([name, data]) => (
                      <span
                        key={name}
                        className="px-2 py-1 bg-white rounded text-xs border border-emerald-200"
                      >
                        {name}: <strong>{data.count}</strong> Tage (
