...
export default function TenantSelectionDialog({
  ...
  const sortedTenants = useMemo(() => {
    ...
    console.log(
      '[TenantSort] lastActiveTokenId from localStorage:',
      lastActiveTokenId,
      typeof lastActiveTokenId,
    );
    console.log(
      '[TenantSort] tenants:',
      tenants.map((t) => ({ id: t.id, type: typeof t.id, is_active: t.is_active, name: t.name })),
    );
  ...
  const handleActivateTenant = async (tokenId: string | number) => {
    setIsActivating(true);
    setSelectedId(tokenId);
    setError('');

    try {
      const result = (await api.activateTenant(String(tokenId))) as { token?: string } | null;
      ...
      if (result && result.token) {
        ...
        localStorage.setItem('active_token_id', String(tokenId));
  ...
  const handleUseDefaultDatabase = async () => {
    ...
    try {
      ...
      setError((err as Error).message || 'Deaktivierung fehlgeschlagen');
      setIsActivating(false);
      setSelectedId(null);
    }
  };
