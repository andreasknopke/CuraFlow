...
interface OverrideDialogState {
  ...
  resolve: ((result: { confirmed: boolean; reason?: string }) => void) | null;
}

interface UseOverrideValidationOptions {
  user: { email?: string; name?: string; [key: string]: unknown } | null;
  doctors?: Doctor[];
}
 ...
 */
export function useOverrideValidation({ user, doctors = [] }: UseOverrideValidationOptions) {
  ...
  const logOverride = useCallback(
    async ({
      ...
      const formattedDate =
        typeof date === 'string' ? date : format(date, 'dd.MM.yyyy', { locale: de });
  ...
  const requestOverride = useCallback(
    ({
      ...
      return new Promise((resolve) => {
        const formattedDate =
          typeof date === 'string' ? date : format(new Date(date), 'dd.MM.yyyy', { locale: de });
  ...
  const confirmOverride = useCallback(
    async (reason: string) => {
      ...
      // Log the override
      await logOverride({
        doctorId: context.doctorId as number,
        doctorName: context.doctorName as string,
   ...
   */
  const setOverrideDialogOpen = useCallback(
    (open: boolean) => {
      if (!open) {
        cancelOverride();
      }
