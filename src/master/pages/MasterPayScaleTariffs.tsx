...
import type { PayscaleTariff, PayscaleGroup } from '@/types/master';
...
const EMPTY_TARIFF: TariffFormValues = {
...
export default function MasterPayScaleTariffs() {
  ...
  const [editingTariff, setEditingTariff] = useState<PayscaleTariff | null>(null);
  const [form, setForm] = useState<TariffFormValues>(EMPTY_TARIFF);
  ...
  const [editingGroup, setEditingGroup] = useState<PayscaleGroup | null>(null);
  const [groupForm, setGroupForm] = useState<Record<string, string>>({ name: '', description: '' });
  const [applyDialogOpen, setApplyDialogOpen] = useState<boolean>(false);
  const [applyTariffId, setApplyTariffId] = useState<string | null>(null);
  ...
  const { data: groups = [] } = useQuery<PayscaleGroup[]>({
    queryKey: ['master-payscale-groups', expandedTariff],
    queryFn: async () => {
      if (!expandedTariff) return [];
      const res = await api.request(`/api/master/payscale-tariffs/${expandedTariff}/groups`) as { groups: PayscaleGroup[] };
      return res.groups || [];
    },
  ...
  const saveTariffMutation = useMutation<unknown, Error, Record<string, unknown>>({
  ...
  const deleteTariffMutation = useMutation<unknown, Error, string>({
    mutationFn: (id: string) => api.request(`/api/master/payscale-tariffs/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['master-payscale-tariffs'] });
  ...
  // Group save mutation
  const saveGroupMutation = useMutation<unknown, Error, Record<string, unknown>>({
    mutationFn: (data: Record<string, unknown>) => {
      if (editingGroup) {
  ...
  const deleteGroupMutation = useMutation<unknown, Error, string>({
    mutationFn: (groupId: string) => api.request(`/api/master/payscale-tariffs/${expandedTariff}/groups/${groupId}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['master-payscale-groups', expandedTariff] });
  ...
  const applyDefaultsMutation = useMutation<unknown, Error, string>({
    mutationFn: (tariffId: string) => api.request(`/api/master/payscale-tariffs/${tariffId}/apply-defaults`, {
      method: 'POST',
    }),
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ['master-payscale-tariffs'] });
      ...
      toast({
        ...
        description: `${(result as { updated: number; skipped: number }).updated} Mitarbeiter aktualisiert, ${(result as { updated: number; skipped: number }).skipped} mit individuellen Werten übersprungen.`,
      });
    },
    onError: (err: Error) => {
  ...
  const openEdit = (tariff: PayscaleTariff) => {
  ...
  const handleSaveTariff = () => {
    ...
    saveTariffMutation.mutate({
      ...form,
      default_weekly_hours: form.default_weekly_hours !== '' ? parseFloat(String(form.default_weekly_hours)) : null,
      default_vacation_days: form.default_vacation_days !== '' ? parseInt(String(form.default_vacation_days), 10) : null,
    });
  };
  ...
  const openEditGroup = (group: PayscaleGroup) => {
  ...
  const openApplyDialog = async (tariff: PayscaleTariff) => {
  ...
  const toggleExpand = (tariffId: string) => {
            ...
            <DialogDescription>
              {applyPreview ? (
                <div className="space-y-3 pt-2">
                  <p>
                    Für <strong>{String(applyPreview.tariffName ?? '')}</strong> werden folgende Standard-Werte auf alle
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-md p-3 space-y-1">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span><strong>Wochenstunden:</strong> {String(applyPreview.hours ?? '–')}h</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Sun className="w-4 h-4 text-amber-600" />
                      <span><strong>Urlaubstage:</strong> {String(applyPreview.days ?? '–')} Tage</span>
                    </div>
                  </div>
