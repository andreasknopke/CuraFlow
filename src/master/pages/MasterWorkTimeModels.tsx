...
import { Timer, Plus, Pencil, Trash2, Loader2, Clock, Users, Check } from 'lucide-react';
...
interface WorkTimeModelPayload {
  name: string;
  hours_per_week: number | null;
  hours_per_day: number | null;
  is_default: boolean;
...
const EMPTY_MODEL: WorkTimeModelForm = {
  name: '',
  hours_per_week: '',
  hours_per_day: '',
  is_default: false,
...
export default function MasterWorkTimeModels() {
  ...
  const [editingModel, setEditingModel] = useState<WorkTimeModel | null>(null);
  const [form, setForm] = useState<WorkTimeModelForm>(EMPTY_MODEL);

  const { data: models = [], isLoading } = useQuery<WorkTimeModel[]>({
  ...
  const saveMutation = useMutation<unknown, Error, WorkTimeModelPayload>({
    mutationFn: (data) => {
      if (editingModel) {
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Gespeichert',
        description: editingModel ? 'Modell aktualisiert.' : 'Neues Modell erstellt.',
      });
    },
    onError: (err) => {
      toast({
  ...
  const deleteMutation = useMutation<unknown, Error, string>({
    ...
    onError: (err) => {
      toast({
        title: 'Fehler',
        description:
          err.message || 'Löschen fehlgeschlagen. Möglicherweise wird das Modell noch verwendet.',
        variant: 'destructive',
      });
    },
  });
  ...
  const openEdit = (model: WorkTimeModel) => {
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate({
      ...form,
      hours_per_week: form.hours_per_week !== '' ? parseFloat(String(form.hours_per_week)) : null,
      hours_per_day: form.hours_per_day !== '' ? parseFloat(String(form.hours_per_day)) : null,
    });
  };
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingModel
                ? 'Aktualisieren Sie die Modelldaten.'
                : 'Definieren Sie ein neues Arbeitszeitmodell.'}
            </DialogDescription>
          </DialogHeader>
