...
import type { WorkTimeModel } from '@/types/models';
...
const EMPTY_MODEL: WorkTimeModelForm = { name: '', hours_per_week: '', hours_per_day: '', is_default: false, description: '' };
...
export default function MasterWorkTimeModels() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.request(`/api/master/work-time-models/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['master-work-time-models'] });
  ...
  const openEdit = (model: WorkTimeModel) => {
    ...
    setForm({
      name: model.name || '',
      hours_per_week: model.hours_per_week != null ? String(model.hours_per_week) : '',
      hours_per_day: model.hours_per_day != null ? String(model.hours_per_day) : '',
