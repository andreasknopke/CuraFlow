...
export default function MasterCostCenters() {
  ...
  const toggleTenantMutation = useMutation({
    mutationFn: ({ costCenterCode, tenantId, linked }) =>
    ...
    onError: (err) => toast.error(`Verknüpfung fehlgeschlagen: ${err.message}`),
  });

  const handleToggleTenant = (costCenterCode, tenantId, linked) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative w-full max-w-xs">
          ...
          <Input placeholder="Code oder Bezeichnung suchen…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8" />
        </div>
