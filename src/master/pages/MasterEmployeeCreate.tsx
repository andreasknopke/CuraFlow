...
export default function MasterEmployeeCreate() {
  ...
  const { data: tariffs = [] } = useQuery<PayscaleTariff[]>({
  ...
  const { data: groups = [] } = useQuery<PayscaleGroup[]>({
    ...
    queryFn: async () => {
      ...
      const res = (await api.request(`/api/master/payscale-tariffs/${selectedTariffId}/groups`)) as { groups: PayscaleGroup[] };
      return res.groups || [];
    },
  ...
  const createMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => api.request('/api/master/employees', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
    onSuccess: (result) => {
      toast({ title: 'Angelegt', description: 'Neuer Mitarbeiter wurde erstellt.' });
      navigate(`/mitarbeiter/central/${(result as { id: string }).id}`);
    },
    onError: (err: Error) => {
  ...
  const handleSave = () => {
    ...
    if (payload.payscale_level === '' || payload.payscale_level == null) {
      ...
      payload.payscale_level = parseInt(payload.payscale_level as string, 10);
    }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea value={form.notes} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => updateField('notes', e.target.value)}
