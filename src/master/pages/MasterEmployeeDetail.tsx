...
export default function MasterEmployeeDetail() {
  ...
  const { data: employee, isLoading } = useQuery<TenantEmployee | null>({
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">{employee.name || 'Unbekannt'}</h1>
      ...
      <div className="flex items-start gap-2 p-3 rounded-lg bg-indigo-50 border border-indigo-100 text-sm text-indigo-700">
        ...
        <span>
          Die Master-Datenbank ist führend. Änderungen hier werden an den Mandanten{' '}
          <strong>{employee.tenantName || tenantId}</strong> durchgereicht.
        </span>
      </div>
      ...
      <Tabs defaultValue="stammdaten" className="w-full">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-3">
                    <Switch checked={employee.is_active ?? true} disabled={!editMode} />
        ...
        <TabsContent value="vertrag" className="mt-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <FieldRow
                  label="Probezeit bis"
                  icon={CalendarDays}
                  value={employee.probation_end}
                  editable={editMode}
                  type="date"
                />
                <FieldRow
                  label="Personalnummer (Loga)"
                  icon={Hash}
                  value={employee.payroll_id}
                  editable={editMode}
                />
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <FieldRow
                  label="Wochen-Soll (Stunden)"
                  icon={Clock}
                  value={employee.target_hours_per_week}
                  editable={editMode}
                  type="number"
        ...
        <TabsContent value="zeitkonto" className="mt-6 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <MiniCard
              icon={Clock}
              label="Ist aktueller Monat"
              value={employee.current_month_actual ?? '–'}
              suffix="h"
              color="blue"
            />
            <MiniCard
              icon={
                employee.overtime_balance > 0
                  ? TrendingUp
                  : employee.overtime_balance < 0
                    ? TrendingDown
                    : Minus
              ...
              value={
                employee.overtime_balance != null
                  ? `${employee.overtime_balance > 0 ? '+' : ''}${employee.overtime_balance}`
                  : '–'
              }
              suffix="h"
              color={
            ...
            <MiniCard
              ...
              value={employee.month_closed ? 'Abgeschlossen' : 'Offen'}
              color={employee.month_closed ? 'emerald' : 'amber'}
            />
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Soll/Ist-Vergleich und kumulierter Saldo pro Monat</CardDescription>
            </CardHeader>
            <CardContent>
              {(employee.time_accounts?.length ?? 0) === 0 ? (
                <div className="text-center py-10 text-slate-400">
                  ...
                  <p className="text-xs mt-1">
                    Daten werden aus dem Mandanten übernommen, sobald Monatsabschlüsse vorliegen.
                  </p>
                </div>
              ) : (
                      ...
                      {(employee.time_accounts ?? []).map((ta, i) => {
                        ...
                        return (
                          <TableRow key={i}>
                            ...
                            <TableCell
                              className={`text-right font-semibold ${delta >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                            >
                              {delta > 0 ? '+' : ''}
                              {delta.toFixed(1)}
                            </TableCell>
                            ...
                            <TableCell
                              className={`text-right font-bold ${ta.total_balance >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                            >
                              {ta.total_balance > 0 ? '+' : ''}
                              {ta.total_balance?.toFixed(1)}
                            </TableCell>
                            <TableCell>
                              <Badge
                                variant={ta.is_closed ? 'default' : 'outline'}
                                className="text-xs"
                              >
...
function AbsenceTypeBadge({ type }: AbsenceTypeBadgeProps) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded text-xs font-medium ${map[type] || 'bg-slate-100 text-slate-800'}`}
    >
      {type}
    </span>
  );
