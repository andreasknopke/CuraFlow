...
import type { HolidaySettings, HolidayEntry, CustomHolidayEntry } from '@/types/master';
...
export default function MasterHolidays() {
  ...
  const { data: settingsData, isLoading: isLoadingSettings } = useQuery({
    ...
    queryFn: () => api.request('/api/master/holidays/settings') as Promise<{ settings: HolidaySettings; states: Record<string, string> }>,
  });

  const { data: customHolidays = [], isLoading: isLoadingCustom } = useQuery({
    queryKey: ['masterCustomHolidays'],
    queryFn: () => api.request('/api/master/holidays/custom') as Promise<CustomHolidayEntry[]>,
  });

  const { data: previewData, isLoading: isLoadingPreview } = useQuery({
    queryKey: ['masterHolidayPreview', previewYear],
    queryFn: () => api.request(`/api/holidays?year=${previewYear}`) as Promise<{ public: HolidayEntry[]; school: HolidayEntry[]; schoolRemovals: Array<{ start: string; end: string }> }>,
    staleTime: 0,
  });

  const settings = (settingsData?.settings || {}) as HolidaySettings;
  ...
  const updateSettingMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => api.request('/api/master/holidays/settings', {
      method: 'PUT',
  ...
  const createHolidayMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => api.request('/api/master/holidays/custom', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
    onSuccess: () => {
  ...
  const deleteHolidayMutation = useMutation({
    mutationFn: (id: string) => api.request(`/api/master/holidays/custom/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['masterCustomHolidays'] });
      queryClient.invalidateQueries({ queryKey: ['masterHolidayPreview'] });
  ...
  const MiniMonth = ({ year, month }: { year: number; month: number }) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const schoolRange = schoolHolidays.find((r: HolidayEntry) => dateStr >= (r.start || '') && dateStr <= (r.end || ''));
                  ...
                  {schoolHolidays.length === 0 ? (
                    ...
                    schoolHolidays.map((h, i) => (
                      <div key={i} className="flex items-center justify-between text-xs py-1 border-b border-slate-100 last:border-0">
                        <span className="text-slate-700">{h.name}</span>
                        <span className="text-slate-500 tabular-nums">
                          {format(parseISO(h.start as string), 'dd.MM.')} – {format(parseISO(h.end as string), 'dd.MM.')}
                        </span>
                      </div>
                    ))
                  )}
