...
export default function MasterHolidays() {
  ...
  const [previewYear, setPreviewYear] = useState<number>(currentYear);
  ...
  // Central settings
  const { data: settingsData, isLoading: isLoadingSettings } = useQuery<SettingsData>({
    queryKey: ['masterHolidaySettings'],
    queryFn: () => api.request('/api/master/holidays/settings'),
  });

  // Custom corrections
  const { data: customHolidays = [], isLoading: isLoadingCustom } = useQuery<CustomHoliday[]>({
    queryKey: ['masterCustomHolidays'],
    queryFn: () => api.request('/api/master/holidays/custom'),
  });

  // Preview resolved holidays
  const { data: previewData, isLoading: isLoadingPreview } = useQuery<HolidayPreviewData>({
  ...
  const updateSettingMutation = useMutation<unknown, Error, SettingMutationVars>({
    mutationFn: ({ key, value }) =>
      api.request('/api/master/holidays/settings', {
        method: 'PUT',
        body: JSON.stringify({ key, value }),
      }),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['masterHolidaySettings'] });
  ...
  const createHolidayMutation = useMutation<unknown, Error, NewHolidayForm>({
    mutationFn: (data) =>
      api.request('/api/master/holidays/custom', {
        method: 'POST',
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: 'Der Eintrag wurde zentral hinzugefügt.',
      });
    },
    onError: (err) => toast({ title: 'Fehler', description: err.message, variant: 'destructive' }),
  });

  const deleteHolidayMutation = useMutation<unknown, Error, string>({
  ...
  const handleAddHoliday = () => {
    if (!newHoliday.name || !newHoliday.start_date) {
      toast({
        ...
        description: 'Name und Startdatum sind erforderlich.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const MiniMonth = ({ year, month }: { year: number; month: number }) => {
      ...
      <div className="space-y-1">
        ...
        <div className="grid grid-cols-7 gap-0.5 text-center">
          {['Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa', 'So'].map((d) => (
            <div key={d} className="text-[10px] text-slate-400 font-medium w-7">
              {d}
            </div>
          ))}
          {days}
        ...
        <CardContent className="py-3 flex items-start gap-3">
          ...
          <div className="text-sm text-indigo-800">
            ...
            <p className="text-indigo-600 mt-0.5">
              Feiertage und Schulferien werden automatisch über die OpenHoliday API geladen. Hier
              können Sie das Bundesland wählen und bei Bedarf Korrekturen vornehmen (z.B. fehlende
              Feiertage ergänzen oder fehlerhafte API-Einträge entfernen).
            </p>
          </div>
        </CardContent>
      </Card>
          ...
          <Card>
            ...
            <CardContent>
              <Select
                value={federalState}
                onValueChange={(val) =>
                  updateSettingMutation.mutate({ key: 'federal_state', value: val })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <CardContent>
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                ...
                {customHolidays.map((h) => (
                  <div
                    key={h.id}
                    className="flex items-center justify-between text-sm border p-2 rounded bg-white"
                  >
                    <div>
                      <div className="font-medium flex items-center gap-2">
                        {h.name}
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded ${h.action === 'add' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                        >
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">
                        ...
                        {h.end_date &&
                          ...
                          ` – ${format(parseISO(h.end_date), 'dd.MM.yyyy')}`}
                      </div>
                    </div>
                    <Button
                      variant="ghost"
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-xs mt-1">
                    {states[federalState] || federalState} · {publicHolidays.length} Feiertage ·{' '}
                    {schoolHolidays.length} Ferienperioden
                  </CardDescription>
                </div>
                  ...
                  {schoolHolidays.length === 0 ? (
                    ...
                    schoolHolidays.map((h, i) => (
                      ...
                      >
                        <span className="text-slate-700">{h.name}</span>
                        <span className="text-slate-500 tabular-nums">
                          {format(parseISO(h.start), 'dd.MM.')} –{' '}
                          {format(parseISO(h.end), 'dd.MM.')}
                        </span>
                      </div>
                    ))
                  )}
