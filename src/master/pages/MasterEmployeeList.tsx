...
interface StatCardProps {
  ...
  label: string;
  value: string | number;
  color?: 'slate' | 'emerald' | 'amber' | 'indigo' | 'red';
}
...
export default function MasterEmployeeList() {
  ...
  const [viewMode, setViewMode] = useState<string>('central');
  ...
  const { data: centralData, isLoading: centralLoading } = useQuery<CentralData>({
  ...
  const { data: staffData, isLoading: staffLoading } = useQuery<StaffData>({
  ...
  const filteredCentral = useMemo(() => {
    ...
    // Sortierung
    const cmp = (a: CentralEmployee, b: CentralEmployee) => {
  ...
  const filteredLegacy = useMemo(() => {
    ...
    const cmp = (a: LegacyStaff, b: LegacyStaff) => {
      let va: string | number, vb: string | number;
      switch (sortField) {
        case 'name':
  ...
  const importMutation = useMutation<ImportResult, Error, ImportItem[]>({
    mutationFn: (items) =>
    ...
    onSuccess: async (res) => {
      ...
      if (imported > 0) {
        toast.success(
          `${imported} von ${total} Mitarbeiter(n) importiert${skipped ? `, ${skipped} übersprungen` : ''}`,
        );
      } else if (skipped > 0) {
  ...
  const handleDelete = (emp: CentralEmployee, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleImportSingle = (staff: LegacyStaff) => {
  ...
  const handleImportAll = () => {
    const items = (filteredList as LegacyStaff[]).map((s) => ({
  ...
  const displayName = (emp: CentralEmployee | LegacyStaff): string => {
    const e = emp as CentralEmployee;
    if (e.first_name && e.last_name) return `${e.first_name} ${e.last_name}`;
    if (e.last_name) return e.last_name;
    return emp.name || '–';
  };
          ...
          {isLoading ? (
            ...
            <ScrollArea className="h-[600px]">
              <Table>
                ...
                <TableBody>
                  {(filteredList as CentralEmployee[]).map((emp) => (
                    <TableRow
                      key={emp.id}
                    ...
                    >
                      <TableCell>
                        <div>
                          ...
                          {!emp.first_name && (
                            <Badge
                              variant="outline"
                              className="ml-2 text-[10px] text-amber-600 border-amber-200"
                            >
                              Name prüfen
                            </Badge>
                          )}
                      ...
                      <TableCell>
                        <span
                          className={`inline-flex items-center gap-1.5 text-xs font-medium ${emp.is_active ? 'text-emerald-700' : 'text-red-500'}`}
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${emp.is_active ? 'bg-emerald-500' : 'bg-red-400'}`}
                          />
                ...
                <TableBody>
                  {(filteredList as LegacyStaff[]).map((staff, i) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <span
                          className={`inline-flex items-center gap-1.5 text-xs font-medium ${staff.is_active ? 'text-emerald-700' : 'text-slate-400'}`}
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${staff.is_active ? 'bg-emerald-500' : 'bg-slate-300'}`}
                          />
...
function StatCard({ icon: Icon, label, value, color = 'slate' }: StatCardProps) {
