...
export default function MasterCentralEmployeeDetail() {
  ...
  const [form, setForm] = useState<EmployeeForm>({} as EmployeeForm);

  // Zentrale Mitarbeiterdaten laden
  const { data: employee, isLoading } = useQuery<CentralEmployee>({
  ...
  useEffect(() => {
    if (employee) {
      setForm({
        ...
        target_hours_per_week: String(employee.target_hours_per_week ?? ''),
        vacation_days_annual: String(employee.vacation_days_annual ?? ''),
  ...
  // Speichern
  const saveMutation = useMutation<unknown, Error, EmployeeForm>({
    mutationFn: (data: EmployeeForm) =>
      api.request(`/api/master/employees/${employeeId}`, {
        method: 'PUT',
  ...
  const displayName =
    [employee.first_name, employee.last_name].filter(Boolean).join(' ') || 'Unbekannt';
  const currentModel = models.find(
    (m) => m.id === (form.work_time_model_id || employee.work_time_model_id),
  );
        ...
        <TabsContent value="vertrag" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Arbeitsvertrag, Arbeitszeitmodell und Urlaubsanspruch
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {editMode ? (
                    <Select
                      value={form.work_time_model_id || '__none__'}
                      onValueChange={(v) =>
                        updateField('work_time_model_id', v === '__none__' ? '' : v)
                      }
                    >
                      <SelectTrigger>
                    ...
                    <p className="text-sm">
                      {currentModel
                        ? `${currentModel.name} (${currentModel.hours_per_week}h/W)`
                        : '–'}
                    </p>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <FieldRow
                  ...
                  editMode={editMode}
                  type="date"
                  onChange={(v) => updateField('probation_end', v)}
                />
              </div>
              <Separator />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {(employee.assignments || []).length === 0 ? (
                <div className="text-center py-12 text-slate-400">
                  ...
                  <p className="text-sm mt-1">
                    Verknüpfen Sie diesen Mitarbeiter mit einem Mandanten über die Mitarbeiterliste
                    des Mandanten.
                  </p>
                </div>
              ) : (
                    ...
                    {employee.assignments.map((a) => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell className="text-sm">
                          {a.assigned_since
                            ? new Date(a.assigned_since).toLocaleDateString('de-DE')
                            : '–'}
                        </TableCell>
                        <TableCell className="text-sm">
            ...
            <CardContent>
              {(employee.timeAccounts || []).length === 0 ? (
                <div className="text-center py-12 text-slate-400">
                  ...
                  <p className="text-sm mt-1">
                    Sobald Schichten geplant werden, entstehen hier automatisch die
                    Zeitkonto-Einträge.
                  </p>
                </div>
              ) : (
                        ...
                        return (
                          <TableRow key={ta.id}>
                            ...
                            <TableCell className="text-right text-sm">
                              <span
                                className={`inline-flex items-center gap-1 font-medium ${isPositive ? 'text-emerald-600' : isNegative ? 'text-red-600' : 'text-slate-500'}`}
                              >
                                {isPositive ? (
