...
export default function MasterAbsences() {
  ...
  const { data: absenceData, isLoading } = useQuery<AbsenceData>({
  ...
  const entries: AbsenceEntry[] = absenceData?.entries ?? [];
  const summary: Record<string, number> = absenceData?.summary ?? {};
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Detaillierte Auflistung aller Abwesenheiten</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <ScrollArea className="h-[500px]">
              <Table>
                ...
                <TableBody>
                  {entries.map((entry, i) => {
                    const config = ABSENCE_TYPES[entry.type] || {
