...
import type { PPUGVStation } from '@/types/master';
...
interface PPUGVRow {
  ...
  patienten: number;
  belegung: number;
  pflegekraefte_ist: number;
  hebammen_ist: number;
  hilfskraefte_ist: number;
  is_estimated?: boolean;
  frostung?: string;
  fabname?: string;
  fabschluessel?: string;
  pfl_sen_ber?: string;
  betten?: number;
  anzahl?: number;
  [key: string]: unknown;
...
interface PPUGVFabSchichtDetail {
  pflege: number;
  hilfe: number;
  hebamme: number;
  patienten: number;
...
interface PPUGVFabMonat {
  monat: string;
  tag: PPUGVFabSchichtDetail;
  nacht: PPUGVFabSchichtDetail;
}

interface PPUGVFabEntry {
  fabschluessel: string | number;
  fabname: string;
  monate: PPUGVFabMonat[];
}

interface PPUGVFabResponse {
  fabs: PPUGVFabEntry[];
}

interface PPUGVTrendMonat {
  monat: string;
  [key: string]: string | number;
}

interface PPUGVTrendResponse {
  monate: PPUGVTrendMonat[];
  years: string[];
}

interface PPUGVQualityYear {
  jahr: number;
  komplett: boolean;
  erwartet: number;
  rowCount: number;
...
interface PPUGVQualityResponse {
  years: PPUGVQualityYear[];
}
...
interface PPBVRow {
  ...
  monat: string;
  schicht: string;
  fachkraefte_soll: number;
  fachkraefte_ist: number;
  ausfall_soll_1: number;
  ausfall_soll_2: number;
  ausfall_soll_3: number;
  azubi_ist: number;
  is_estimated?: boolean;
  [key: string]: unknown;
}

interface PPBVDataResponse {
  data: PPBVRow[];
  meta?: PPUGVMeta;
  building?: boolean;
  estimatedCount?: number;
...
export default function MasterPPUGV() {
  ...
  // Stationen laden
  const { data: stationsData } = useQuery<{ stations: PPUGVStation[] }>({
    queryKey: ['ppugv-stations'],
    queryFn: async () => {
      try {
        return await api.request('/api/master/ppugv/stations') as Promise<{ stations: PPUGVStation[] }>;
      } catch {
        return { stations: [] };
      }
    },
  ...
  const { data, isLoading, isError, error } = useQuery<PPUGVDataResponse>({
    ...
    queryFn: async () => {
      ...
      const result = await api.request(`/api/master/ppugv${qs ? `?${qs}` : ''}`) as PPUGVDataResponse;
  ...
  const refreshMutation = useMutation<void, Error, void>({
    mutationFn: async () => {
      setRefreshStarted(true);
      return await api.request('/api/master/ppugv/refresh', { method: 'POST' }) as Promise<void>;
    },
    onSettled: () => {
  ...
  const { data: fabData } = useQuery<PPUGVFabResponse>({
    ...
    queryFn: async () => {
      ...
      return await api.request(url) as Promise<PPUGVFabResponse>;
    },
    enabled: activeTab === 'fab',
  });

  // Trend-Daten (Jahresvergleich)
  const { data: trendData, isLoading: trendLoading } = useQuery<PPUGVTrendResponse>({
    ...
    queryFn: async () => {
      ...
      if (showFutureMonths) params.set('include_future', 'true');
      return await api.request(`/api/master/ppugv/trends?${params.toString()}`) as Promise<PPUGVTrendResponse>;
    },
    enabled: activeTab === 'trends',
  });

  // Datenqualitaets-Check
  const { data: qualityData } = useQuery<PPUGVQualityResponse>({
    queryKey: ['ppugv-quality'],
    queryFn: async () => await api.request('/api/master/ppugv/quality') as Promise<PPUGVQualityResponse>,
  });

  // Export-Download
  const downloadExport = useCallback(async (format: 'inek' | 'csv') => {
    try {
      ...
      console.error('[PPUGV-Export]', (err as Error).message);
  ...
  const { data: ppbvData, isLoading: ppbvLoading } = useQuery<PPBVDataResponse>({
    ...
    queryFn: async () => {
      ...
      if (showFutureMonths) params.set('include_future', 'true');
      const qs = params.toString();
      return await api.request(`/api/master/ppbv${qs ? `?${qs}` : ''}`) as Promise<PPBVDataResponse>;
    },
  ...
  // PPBV-Refresh
  const ppbvRefreshMutation = useMutation<void, Error, void>({
    mutationFn: async () => await api.request('/api/master/ppbv/refresh', { method: 'POST' }) as Promise<void>,
    onSettled: () => {
  ...
  // PPBV-Download
  const downloadPpbvExport = useCallback(async (format: 'inek' | 'csv') => {
    try {
      ...
      console.error('[PPBV-Export]', (err as Error).message);
      alert('PPBV-Export fehlgeschlagen: ' + (err as Error).message);
    }
  }, [selectedYear]);
  ...
  const chartData = useCallback(() => {
    const monthMap = new Map<string, {
      ...
      hilfe_nacht: number;
      hebamme_tag: number;
      hebamme_nacht: number;
      belegung_tag: number;
      belegung_nacht: number;
      patienten_tag: number;
      patienten_nacht: number;
      is_estimated: boolean;
    }>();

    let filteredRows = rows;
    if (selectedStation !== 'all') {
      filteredRows = rows.filter((r: PPUGVRow) => r.stationsname === selectedStation);
    }
  ...
  const fabRows = useMemo(() => {
    ...
    return fabData.fabs.filter((f: PPUGVFabEntry) => String(f.fabschluessel) === String(selectedFab));
  }, [fabData, selectedFab]);

  const fabOptions = useMemo(() => {
    if (!fabData?.fabs) return [];
    return fabData.fabs.map((f: PPUGVFabEntry) => ({
  ...
  const trendLines = useMemo(() => {
    ...
    return data.map((m: PPUGVTrendMonat) => {
      const entry: Record<string, string | number> = { monat: m.monat };
      yrs.forEach((y: string) => { entry[`${y}`] = Number(m[`${trendMetric}_${y}`] || 0); });
      return entry;
    });
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'stations' && (
        <>
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={selectedStation} onValueChange={setSelectedStation}>
                ...
                <SelectContent>
                  ...
                  {stations.map((s: PPUGVStation) => (
                    <SelectItem key={s.stationsname as string} value={s.stationsname as string}>{s.stationsname as string}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
                  ...
                  {chartMode === 'staffing' ? (
                    <BarChart data={chartData()}>
                      ...
                      <Bar dataKey="pflege_tag" name="Pflegekräfte (Tag)" fill={CHART_COLORS.pflegekraefte} radius={[2, 2, 0, 0]}>
                        {chartData().map((entry: { is_estimated: boolean }, idx: number) => (
          ...
          {data?.estimatedCount != null && data.estimatedCount > 0 && (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {data?.count ?? 0} Datensätze {data?.estimatedCount != null && data.estimatedCount > 0 ? `(davon ${data.estimatedCount} als Vorjahresdaten geschätzt)` : ''} – Tag- und Nachtschicht pro Station und Monat
              </CardDescription>
            </CardHeader>
            <CardContent>
              {rows.length === 0 && !isBuilding ? (
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {rows.map((row: PPUGVRow, idx: number) => (
                        <TableRow key={row.id as string | number || idx} className={row.is_estimated ? 'bg-amber-50/50' : ''}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={selectedFab} onValueChange={setSelectedFab}>
                ...
                <SelectContent>
                  ...
                  {fabOptions.map((f: { value: string; label: string }) => (
            ...
            <CardContent>
              {!fabData ? (
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={fabRows.map((fab: PPUGVFabEntry) => ({
                    name: fab.fabname,
                    pflege: fab.monate.reduce((s: number, m: PPUGVFabMonat) => s + m.tag.pflege + m.nacht.pflege, 0) / Math.max(fab.monate.length, 1),
                    hilfe: fab.monate.reduce((s: number, m: PPUGVFabMonat) => s + m.tag.hilfe + m.nacht.hilfe, 0) / Math.max(fab.monate.length, 1),
                    hebamme: fab.monate.reduce((s: number, m: PPUGVFabMonat) => s + m.tag.hebamme + m.nacht.hebamme, 0) / Math.max(fab.monate.length, 1),
                  }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {fabRows.flatMap((fab: PPUGVFabEntry) =>
                        fab.monate.map((m: PPUGVFabMonat, mi: number) => (
              ...
              {trendLoading ? (
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <LineChart data={trendLines}>
                    ...
                    <Tooltip />
                    <Legend />
                    {trendYears.map((y: string, i: number) => (
        ...
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{selectedYear} – Ø VK pro Monat (Tag + Nacht gemittelt){ppbvData?.estimatedCount != null && ppbvData.estimatedCount > 0 ? ` – ${ppbvData.estimatedCount} geschätzte Datensätze` : ''}</CardDescription>
            </CardHeader>
            <CardContent>
              {ppbvLoading ? (
                <div className="flex items-center justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-indigo-500" /></div>
                ...
                <>
                  <ResponsiveContainer width="100%" height={350}>
                    <BarChart data={MONTH_ORDER.map((m: string) => {
                      const monthRows = ppbvRows.filter((r: PPBVRow) => r.monat === m);
                      const tag = monthRows.filter((r: PPBVRow) => r.schicht === 'Tag');
                      const nacht = monthRows.filter((r: PPBVRow) => r.schicht === 'Nacht');
                      return {
                        name: m,
                        is_estimated: monthRows.some((r: PPBVRow) => r.is_estimated),
                        soll: tag.reduce((s: number, r: PPBVRow) => s + Number(r.fachkraefte_soll || 0), 0) / Math.max(tag.length, 1),
                        ist: tag.reduce((s: number, r: PPBVRow) => s + Number(r.fachkraefte_ist || 0), 0) / Math.max(tag.length, 1),
                        sollNacht: nacht.reduce((s: number, r: PPBVRow) => s + Number(r.fachkraefte_soll || 0), 0) / Math.max(nacht.length, 1),
                        istNacht: nacht.reduce((s: number, r: PPBVRow) => s + Number(r.fachkraefte_ist || 0), 0) / Math.max(nacht.length, 1),
                      };
                    })}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                      <XAxis dataKey="name" tick={{ fontSize: 12 }} />
                      ...
                      <Bar dataKey="soll" name="Pflege Soll (Tag)" fill="#94a3b8" radius={[2, 2, 0, 0]}>
                        {MONTH_ORDER.map((m: string, idx: number) => {
                          const monthRows = ppbvRows.filter((r: PPBVRow) => r.monat === m);
                          const estimated = monthRows.some((r: PPBVRow) => r.is_estimated);
                  ...
                  {ppbvData?.estimatedCount != null && ppbvData.estimatedCount > 0 && (
                  ...
                  <div className="mt-8">
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={MONTH_ORDER.map((m: string) => {
                        const monthRows = ppbvRows.filter((r: PPBVRow) => r.monat === m && r.schicht === 'Tag');
                        const estimated = monthRows.some((r: PPBVRow) => r.is_estimated);
                        const avg = (field: string) => monthRows.reduce((s: number, r: PPBVRow) => s + Number(r[field] || 0), 0) / Math.max(monthRows.length, 1);
                        return {
                          name: m,
                        ...
                        <Bar dataKey="urlaub" name="Wochenfeiertage/Urlaub" fill="#f59e0b" stackId="a" radius={[2, 2, 0, 0]}>
                          {MONTH_ORDER.map((m: string, idx: number) => {
                            const estimated = ppbvRows.some((r: PPBVRow) => r.monat === m && r.is_estimated);
                            return <Cell key={idx} fillOpacity={estimated ? 0.35 : 1} />;
                          })}
                        </Bar>
          ...
          {ppbvRows.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {ppbvRows.length} Datensätze {ppbvData?.estimatedCount != null && ppbvData.estimatedCount > 0 ? `(davon ${ppbvData.estimatedCount} als Vorjahresdaten geschätzt)` : ''} – InEK-Excel enthält alle 26 Spalten
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {ppbvRows.map((row: PPBVRow, idx: number) => {
            ...
            {rows.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {rows.map((row: PPUGVRow, idx: number) => (
                      <TableRow key={row.id as string | number || idx} className="text-xs">
