...
import type { Tenant, TimeTrackingEntry, TimeTrackingSummary } from '@/types/master';
...
export default function MasterTimeTracking() {
  ...
  const { data: timeData, isLoading } = useQuery<{ entries: TimeTrackingEntry[]; summary: TimeTrackingSummary | null }>({
    ...
    queryFn: async () => {
      try {
        ...
        return (await api.request(`/api/master/time-tracking?${params}`)) as { entries: TimeTrackingEntry[]; summary: TimeTrackingSummary | null };
      } catch {
...
function StatCard({ label, value, color = 'slate' }: { label: string; value: string | number; color?: 'slate' | 'blue' | 'emerald' | 'red' }) {
