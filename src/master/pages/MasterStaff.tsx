...
import type { Tenant, StaffEntry } from '@/types/master';
...
export default function MasterStaff() {
  ...
  const { data: staffData, isLoading } = useQuery<{ staff: StaffEntry[] }>({
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Stammdaten aus allen Mandanten-Datenbanken
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                <TableBody>
                  {staffList.map((staff, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell>
                        <span className={`inline-flex items-center gap-1 text-xs ${staff.is_active ? 'text-emerald-600' : 'text-slate-400'}`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${staff.is_active ? 'bg-emerald-500' : 'bg-slate-300'}`} />
