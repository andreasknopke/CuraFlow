...
import DoctorForm, { type DoctorFormProps } from '@/components/staff/DoctorForm';
...
export default function StaffPage() {
  ...
  const [editingDoctor, setEditingDoctor] = useState<Record<string, unknown> | null>(null);
  ...
  const { data: doctors = [], isLoading } = useQuery<any[], Error, any[]>({
    queryKey: ['doctors'],
  ...
  const { data: colorSettings = [] } = useQuery<any[]>({
    queryKey: ['colorSettings'],
    queryFn: () => db.ColorSetting.list() as Promise<any[]>,
  });
  ...
  const getRoleColor = (role: string) => {
    ...
    const match = defaults[role as keyof typeof defaults];
  ...
  const createMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      db.Doctor.create({ ...data, order: doctors.length }),
    onSuccess: () => {
      trackDbChange();
  ...
  const handleSave = (data: Record<string, unknown>) => {
    if (editingDoctor) {
      updateMutation.mutate({ id: editingDoctor.id as string, data });
    } else {
      createMutation.mutate(data);
    }
  };

  const handleEdit = (doctor: Record<string, unknown>) => {
  ...
  return (
    <div className="container mx-auto max-w-6xl">
                    ...
                    {doctors.map((doctor, index) => (
                      ...
                      >
                        {(provided, snapshot) => (
                          ...
                          >
                            <Card
                              className={`hover:shadow-md ${snapshot.isDragging ? 'shadow-lg ring-2 ring-indigo-500' : ''}`}
                            >
      ...
      <DoctorForm
        ...
        doctor={editingDoctor as DoctorFormProps['doctor']}
        onSubmit={handleSave}
      />
    </div>
  );
