...
import type { ShiftEntry } from '@/types/models';
...
const getWishDateRange = (wish: { date?: string; [key: string]: unknown }) => {
  const start = getWishStartDate(wish) || wish?.date;
  const end = getWishEndDate(wish) || wish?.date;
  return { start, end };
};

const expandWishDates = (wish: { date?: string; [key: string]: unknown }): string[] => {
...
interface AdminTasksSectionProps {
  allPendingWishes: WishData[];
  isLoadingPending: boolean;
  doctors: DoctorRecord[];
  handleQuickDecision: (wish: WishData, action: string) => void;
  silentDeleteWishMutation: { mutate: (id: string) => void; isPending: boolean };
  isUpdating: boolean;
}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
              {visibleWishes.map((wish) => {
                ...
                return (
                  <div
                    key={wish.id}
                    className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between gap-2 min-w-0"
                  >
                    <div>
                      ...
                      <div className="text-sm text-slate-600 mb-2">
                        ...
                        <span
                          className={wish.type === 'service' ? 'text-green-600' : 'text-red-600'}
                        >
                          {wish.type === 'service'
                            ? wish.position
                              ? `Dienst: ${wish.position}`
                              : 'Dienstwunsch'
                            : 'Kein Dienst'}
                        </span>
                      </div>
...
export default function MyDashboardPage() {
  ...
  const { data: doctors = [], isLoading: isLoadingDocs } = useQuery<DoctorRecord[]>({
    queryKey: ['doctors'],
    queryFn: () => db.Doctor.list() as Promise<DoctorRecord[]>,
  });
  ...
  useEffect(() => {
    if (doctorsForSelection.length > 0 && user && !selectedDoctorId) {
      ...
      if (user.role === 'admin') {
        if (user.doctor_id && doctorsForSelection.some((d) => d.id === String(user.doctor_id))) {
          setSelectedDoctorId(String(user.doctor_id));
        } else {
  ...
  const { data: shifts = [], isLoading: isLoadingShifts } = useQuery<ShiftEntry[]>({
    ...
    queryFn: async () => {
      ...
      }) as Promise<ShiftEntry[]>; // Get next 100 shifts sorted by date
    },
    enabled: !!selectedDoctorId,
  });

  const { data: wishes = [], isLoading: isLoadingWishes } = useQuery<WishData[]>({
  ...
  const { data: dashboardWorkplaces = [] } = useQuery<WorkplaceRecord[]>({
    queryKey: ['workplaces'],
    queryFn: () => db.Workplace.list() as Promise<WorkplaceRecord[]>,
    staleTime: 1000 * 60 * 5,
  });

  // Dynamic set of service position names
  const servicePositionNames = React.useMemo(() => {
    const names = dashboardWorkplaces.filter((w) => w.category === 'Dienste').map((w) => w.name);
    return names.length > 0 ? names : ['Dienst Vordergrund', 'Dienst Hintergrund', 'Spätdienst'];
  }, [dashboardWorkplaces]);

  // User: Fetch Shift Notifications
  const { data: notifications = [], isLoading: _isLoadingNotifications } = useQuery<
    ShiftNotification[]
  >({
    ...
    queryFn: async () => {
      ...
      return db.ShiftNotification.filter({
        doctor_id: selectedDoctorId,
        acknowledged: false,
      }) as Promise<ShiftNotification[]>;
    },
    enabled: !!selectedDoctorId,
  });

  const acknowledgeNotificationMutation = useMutation({
    mutationFn: (id: string) => db.ShiftNotification.update(id, { acknowledged: true }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['shiftNotifications'] }),
  });
  ...
  const bulkAcknowledgeMutation = useMutation({
    mutationFn: async () => {
      ...
      const allUnack = (await db.ShiftNotification.filter({
        doctor_id: selectedDoctorId,
        acknowledged: false,
      })) as ShiftNotification[]; // High limit to get everything

      if (allUnack.length > 0) {
        await Promise.all(
          allUnack.map((n) => db.ShiftNotification.update(n.id, { acknowledged: true })),
        );
      }
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['shiftNotifications'] }),
  ...
  const updateDoctorMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      db.Doctor.update(selectedDoctorId as string, data),
  ...
  const { data: allPendingWishes = [], isLoading: isLoadingPending } = useQuery<WishData[]>({
    queryKey: ['allPendingWishes'],
    queryFn: async () => {
      const pending = (await db.WishRequest.filter({ status: 'pending' })) as WishData[];
      const cancellation = (await db.WishRequest.filter({
  ...
  const holidayCalculatorsByYear = useMemo(() => new Map<number, HolidayCalculator>(), []);

  // Mark unviewed items as viewed when user visits dashboard
  useEffect(() => {
    if (isAuthenticated && !isAdmin && user?.doctor_id) {
      db.WishRequest.filter({ doctor_id: user.doctor_id, user_viewed: false }).then(
        (rawUnviewed) => {
          const unviewed = rawUnviewed as WishData[];
          if (unviewed.length > 0) {
            Promise.all(
              unviewed.map((w) => db.WishRequest.update(w.id, { user_viewed: true })),
            ).then(() => {
              queryClient.invalidateQueries({ queryKey: ['wishes'] });
              queryClient.invalidateQueries({ queryKey: ['dashboardAlert'] });
            });
          }
        },
      );
  ...
  const getHolidayCalculatorForYear = async (year: number) => {
    if (!holidayCalculatorsByYear.has(year)) {
      const apiData = (await api.getHolidays(year)) as ApiHolidayDataLike;
  ...
  const handleAutoShiftCreation = async (wish: Record<string, unknown>, dateStr: string) => {
    ...
    const existingShifts = (await db.ShiftEntry.filter({
      date: dateStr,
    ...
    const workplaces = (await db.Workplace.list()) as WorkplaceRecord[];
    ...
    if (wp?.auto_off) {
      ...
      const nextDayStr = getAutoFreiDate(
        dateStr,
        (date) => !!holidayCalculator.isPublicHoliday(date),
      );
      if (!nextDayStr) return;

      // Check if next day is free
      const nextDayShifts = (await db.ShiftEntry.filter({
        date: nextDayStr,
        doctor_id: wish.doctor_id,
      })) as ShiftEntry[];
  ...
  const updateWishMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allPendingWishes'] });
      queryClient.invalidateQueries({ queryKey: ['wishes'] });
  ...
  const deleteWishMutation = useMutation({
    mutationFn: async (wish: Record<string, unknown>) => {
      ...
      return db.WishRequest.delete(wish.id as string);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allPendingWishes'] });
      queryClient.invalidateQueries({ queryKey: ['wishes'] });
  ...
  const silentDeleteWishMutation = useMutation({
    mutationFn: (wishId: string) => db.WishRequest.delete(wishId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allPendingWishes'] });
      queryClient.invalidateQueries({ queryKey: ['wishes'] });
    },
  ...
  const handleQuickDecision = (wish: WishData, action: string) => {
  ...
  const upcomingServices = shifts.filter((s) => {
    ...
    if (!d) return false;
    return servicePositionNames.includes(s.position) && (isAfter(d, today) || isSameDay(d, today));
  });
  ...
  const pendingWishes = wishes.filter((w) => {
    ...
    return (
      (w.status === 'pending' || w.status === 'cancellation_requested') && (!end || end >= todayStr)
    );
  });
    ...
    <div className="container mx-auto max-w-7xl space-y-6 px-2 sm:px-4">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
          ...
          {selectedDoctor && (
            <div
              className="flex items-center justify-between gap-3 bg-white p-2 rounded-lg border shadow-sm"
              title={
                !selectedDoctor.email
                  ? 'Keine Benachrichtigungs-E-Mail-Adresse in den Mitarbeiter-Stammdaten hinterlegt.'
                  : 'E-Mail Benachrichtigungen aktivieren'
              }
            >
              <div className="flex items-center gap-2">
                <Mail
                  className={`w-4 h-4 ${selectedDoctor.receive_email_notifications ? 'text-indigo-600' : 'text-slate-400'}`}
                />
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  updateDoctorMutation.mutate({ receive_email_notifications: checked })
                }
              />
            </div>
          )}
          ...
          {isAdmin && (
            <div className="flex items-center gap-2 bg-white p-2 rounded-lg border shadow-sm">
              ...
              <span className="text-sm font-medium text-slate-700 mr-2 hidden sm:inline">
                Ansicht für:
              </span>
      ...
      {isAdmin && (
        <AdminTasksSection
          ...
          isUpdating={
            updateWishMutation.isPending ||
            deleteWishMutation.isPending ||
            silentDeleteWishMutation.isPending
          }
        />
      )}
      ...
      {!selectedDoctorId ? (
        <Card>
          <CardContent className="p-12 text-center text-slate-500">
            {user?.doctor_id
              ? 'Lade Daten...'
              : 'Kein Profil zugeordnet. Bitte wenden Sie sich an einen Administrator.'}
          </CardContent>
        </Card>
      ) : (
            ...
            <CardContent className="flex-1 overflow-auto p-0">
              {isLoadingShifts ? (
                ...
                <div className="p-6 text-center text-slate-500 text-sm">
                  Keine kommenden Dienste geplant.
                </div>
              ) : (
                <Table>
                  <TableBody>
              ...
              {notifications.length > 0 && (
                <div className="bg-amber-50 border-b border-amber-100 p-3 space-y-2">
                  ...
                  {notifications.map((notif) => (
                    <div
                      key={notif.id}
                      className="bg-white p-2 rounded border border-amber-200 shadow-sm text-sm flex justify-between items-start gap-2"
                    >
                      <div>
                        <div className="font-medium text-slate-800">
                          {safeFormatDate(notif.date, 'dd.MM.yyyy')}
                        </div>
                        <div className="text-slate-600 text-xs mt-0.5">{notif.message}</div>
                      </div>
                      <Button
                        size="icon"
                        variant="ghost"
                      ...
                      <TableRow key={shift.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={
                              shift.position === 'Urlaub'
                                ? 'bg-green-50 text-green-700 border-green-200'
                                : shift.position === 'Krank'
                                  ? 'bg-red-50 text-red-700 border-red-200'
              ...
              <div>
                ...
                {isLoadingWishes ? (
                  ...
                  <div className="text-sm text-slate-500 italic">
                    Keine vergangenen Entscheidungen.
                  </div>
                ) : (
                  <div className="space-y-2">
                    {recentDecisions.map((w) => (
                      <div
                        key={w.id}
                        className="flex flex-col p-2 bg-white rounded border border-slate-100 gap-2"
                      >
