...
export default function StatisticsPage() {
  ...
  // 1. Fetch Data
  const { data: doctors = [], isLoading: isLoadingDocs } = useQuery<any[], Error, any[]>({
    queryKey: ['doctors'],
    queryFn: () => db.Doctor.list() as Promise<any[]>,
    select: (data): any[] => data.sort((a, b) => (a.order || 0) - (b.order || 0)),
  });

  const { data: workplaces = [], isLoading: isLoadingWorkplaces } = useQuery<any[]>({
    queryKey: ['workplaces'],
    queryFn: () => db.Workplace.list() as Promise<any[]>,
  });
  ...
  const { data: shifts = [], isLoading: isLoadingShifts } = useQuery<any[]>({
    ...
    queryFn: async (): Promise<any[]> => {
      try {
        ...
        const all = (await db.ShiftEntry.list()) as any[];
        return all.filter((s: any) => s.date.startsWith(year));
      }
    },
  });

  const { data: wishes = [], isLoading: isLoadingWishes } = useQuery<any[]>({
  ...
  const stats = useMemo(() => {
    if (isLoading)
      return {
        byDoctor: [] as any[],
        byMonth: [] as any[],
        totals: {} as { dienste: number; rotationen: number },
        rotationItems: [] as string[],
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Auswertung der Dienste und Arbeitsplatzzuweisungen für {year}
          </p>
        </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList>
          <TabsTrigger value="overview" className="flex items-center gap-2">
            <BarChart3 className="w-4 h-4" /> Übersicht & Charts
          </TabsTrigger>
          ...
          <TabsTrigger value="details" className="flex items-center gap-2">
            <TableIcon className="w-4 h-4" /> Detaillierte Tabelle
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-2">
            <ChartCard
              ...
              description={
                month === 'all'
                  ? `Anzahl der Dienste im Jahr ${year}`
                  : `Anzahl der Dienste im ${MONTHS[parseInt(month)]} ${year}`
              }
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  {stats.serviceItems.map((item, index) => {
                    ...
                    const color =
                      (wp?.service_type && SERVICE_TYPE_COLORS[wp.service_type]) ||
                      COLORS[item] ||
                      `hsl(${index * 60}, 70%, 50%)`;
                    return (
                      <Bar
                        key={item}
                        dataKey={`breakdown.${item}`}
                        name={item}
                        stackId="a"
                        fill={color}
                      />
                ...
                >
                  ...
                  {stats.rotationItems.map((item, index) => (
                    <Bar
                      ...
                      name={item}
                      stackId="a"
                      fill={COLORS[item] || `hsl(${index * 40}, 70%, 50%)`}
                    />
                  ))}
                </BarChart>
              </ResponsiveContainer>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right font-bold border-l bg-blue-50">
                        Σ Dienste
                      </TableHead>
                      {stats.serviceItems.map((item) => (
                        <TableHead key={item} className="text-right text-xs text-slate-500">
                          {item.replace('Dienst ', '')}
                        </TableHead>
                      ))}
                      <TableHead className="text-right font-bold border-l bg-emerald-50">
                        Σ Plätze
                      </TableHead>
                  ...
                  <TableBody>
                    {stats.byDoctor.map((doc) => (
                      <TableRow key={doc.id}>
                        ...
                        <TableCell className="text-right font-bold border-l bg-blue-50/50">
                          {doc.totalDienste}
                        </TableCell>
                        {stats.serviceItems.map((item) => (
                        ...
                        <TableCell className="text-right font-bold border-l bg-emerald-50/50">
                          {doc.totalRotationen}
                        </TableCell>
                        {stats.rotationItems.map((item) => (
                          <TableCell key={item} className="text-right text-slate-600">
