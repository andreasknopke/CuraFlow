...
import type { Doctor, ShiftEntry, StaffingPlanEntry, SystemSetting } from '@/types/models';
import type { Conflict } from '@/components/vacation/ConflictDialog';
import {
  Select,
  SelectContent,
  SelectItem,
...
interface SimulationData {
  newShifts: SimulationShift[];
  shiftsToDelete: SimulationShift[];
  shiftsToDeleteIds: string[];
}
...
export default function VacationPage() {
  ...
  const { data: colorSettings = [] } = useQuery<ColorSettingRow[]>({
    queryKey: ['colorSettings'],
    queryFn: () => db.ColorSetting.list() as Promise<ColorSettingRow[]>,
  });
  ...
  const getPriority = (position: string): number =>
    (ABSENCE_PRIORITY as Record<string, number>)[position] || 0;

  // SIMULATION MODE: Berechnet Änderungen, führt sie aber NICHT aus
  const handleSyncAbsences = () => {
    const newShifts: SimulationShift[] = [];
    const shiftsToDelete: SimulationShift[] = [];
    const shiftsToDeleteIds: string[] = [];
    ...
    doctors.forEach((doc) => {
      ...
      days.forEach((day) => {
        ...
        const available = isDoctorAvailable(doc as any, day, staffingPlanEntries as any);
        ...
        if (!available) {
          ...
          if (reason === 'Unbekannt') {
            ...
            const entry = staffingPlanEntries.find(
              (e) => e.doctor_id === doc.id && e.year === year && e.month === month,
            );
            const val = entry
  ...
  const executeSyncAbsences = () => {
    ...
    const shiftsToCreate = dataToProcess.newShifts.map(
      ({
        doctorName: _doctorName,
        reason: _reason,
        replacesExisting: _replacesExisting,
        ...shift
      }) => shift,
    );
  ...
  const visibleTypes = rawVisibleTypes
    ? JSON.parse(rawVisibleTypes)
    : ['Urlaub', 'Krank', 'Frei', 'Dienstreise', 'Nicht verfügbar'];

  const minPresentSpecialists = parseInt(
    systemSettings.find((s) => s.key === 'min_present_specialists')?.value || '2',
  );
  const minPresentAssistants = parseInt(
    systemSettings.find((s) => s.key === 'min_present_assistants')?.value || '4',
  );
  ...
  const customColors = React.useMemo(() => {
    ...
    colorSettings
      ...
      .forEach((s) => {
        if (
          !colors[s.name ?? ''] ||
          !colorSettings.some((cs) => cs.name === s.name && cs.category === 'absence')
        ) {
          colors[s.name ?? ''] = { backgroundColor: s.bg_color ?? '', color: s.text_color ?? '' };
        }
      });
  ...
  const yearShifts = allShifts.filter(
    (s) => s.doctor_id === selectedDoctorId && absencePositions.includes(s.position),
  );

  const overviewShifts = allShifts.filter((s) => absencePositions.includes(s.position));
  ...
  const deleteShiftMutation = useMutation({
    mutationFn: (id: string) => db.ShiftEntry.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['shifts', selectedYear] });
    },
  });

  const [activeType, setActiveType] = useState<string>('Urlaub');
  ...
  const analyzeConflicts = (days: Date[], targetDoctorId: string, newPosition: string) => {
    ...
    days.forEach((d) => {
      ...
      if (existingShift && existingShift.position !== newPosition) {
        ...
        conflicts.push({
          date: dStr,
          existingShift: existingShift as Conflict['existingShift'],
          newPosition,
          conflictType,
        });
      }
  ...
  const executeRangeAction = (
    days: Date[],
    targetDoctorId: string,
    deleteIds: string[],
    overwriteIds: string[],
    keepOptionalServices: boolean,
  ) => {
    const relevantShifts = allShifts.filter((s) => s.doctor_id === targetDoctorId);
    const shiftsToDeleteIds = [...deleteIds, ...overwriteIds];
    const newShifts: Array<{ date: string; position: string; doctor_id: string }> = [];
  ...
  const handleRangeSelection = (start: Date, end: Date, doctorId?: string | null) => {
    ...
    if (activeType === 'DELETE') {
      ...
      const shiftsToDeleteIds = days
        ...
        .filter((s): s is ShiftEntry => s != null)
        .map((s) => s.id);
  ...
  const handleConflictConfirm = ({
    ...
    if (type === 'range') {
      ...
      const idsToDelete = (data.existingShifts as ShiftEntry[]).map((s) => s.id);
  ...
  const handleToggleShift = (
    ...
    const relevantShifts = doctorId
      ? allShifts.filter((s) => s.doctor_id === targetDoctorId)
      : yearShifts;
    ...
    const isExistingAbsence = [
      'Urlaub',
      'Frei',
      'Krank',
      'Dienstreise',
      'Nicht verfügbar',
    ].includes(existingShift.position);
  ...
  const absenceTypes = React.useMemo(() => {
    const types: Array<{
      ...
      textColor: string;
      isDelete?: boolean;
    }> = ['Urlaub', 'Frei', 'Krank', 'Dienstreise', 'Nicht verfügbar'].map((id) => {
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Übersicht der {absencesCaption} und {availableCaption}
          </p>
        </div>
        ...
        <>
          {selectedDoctor ? (
            <DoctorYearView
              ...
              isSchoolHoliday={(date) => !!isSchoolHoliday(date)}
              isPublicHoliday={(date) => !!isPublicHoliday(date)}
            />
          ) : (
              ...
              {simulationData.newShifts.length === 0 ? (
                <div className="flex-1 flex items-center justify-center text-slate-500">
                  <div className="text-center">
                    ...
                    <p className="text-sm">
                      Alle {absencesCaption} aus dem Stellenplan sind bereits eingetragen.
                    </p>
                  </div>
                </div>
              ) : (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {simulationData.newShifts.slice(0, 500).map((shift, idx) => (
                        <TableRow
                          key={idx}
                          className={shift.replacesExisting ? 'bg-amber-50' : 'bg-green-50'}
                        >
                          <TableCell>
                          ...
                          <TableCell>
                            {format(new Date(shift.date), 'dd.MM.yyyy (EEEEEE)', {
                              locale: undefined,
                            })}
                          </TableCell>
                          <TableCell>
              ...
              {simulationData.newShifts.length > 0 && (
                <div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(
                      simulationData.newShifts.reduce(
                        (acc, shift) => {
                      ...
                      .map(([name, count]) => (
                        <span
                          key={name}
                          className="px-2 py-1 bg-white rounded text-sm border border-blue-200"
                        >
