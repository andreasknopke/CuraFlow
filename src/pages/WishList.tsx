...
import type { Doctor, ShiftEntry, WishRequest, Workplace } from '@/types/models';

export default function WishListPage() {
  const { isReadOnly: _isReadOnly, isAuthenticated, user } = useAuth();
  ...
  const [dialogState, setDialogState] = useState<{
    isOpen: boolean;
    date: Date | null;
    wish: Record<string, unknown> | null;
  }>({
    isOpen: false,
    date: null,
    wish: null,
  ...
  const logWishAction = (action: string, doctorName: string, date: string, type: string) => {
    if (!user) return;
    const typeLabel =
      type === 'service' ? 'Dienstwunsch' : type === 'no_service' ? 'Kein Dienst' : 'Löschung';
    db.SystemLog.create({
      level: 'wish_request',
      source: 'Wunschkiste',
  ...
  const handleDateClick = (date: Date) => {
    ...
    if (existingShift && absencePositions.includes(existingShift.position)) {
      alert(
        `Am ${format(date, 'dd.MM.yyyy')} ist bereits eine Abwesenheit eingetragen (${existingShift.position}).`,
      );
      return;
    }
    ...
    setDialogState({
      isOpen: true,
      date: date,
      wish: (existingWish as unknown as Record<string, unknown>) || null,
    });
  };

  // Helper: Create shift from approved wish
  const createShiftFromWish = async (doctorId: string, date: string, position: string) => {
  ...
  const handleDialogSave = async (formData: Record<string, unknown>) => {
    ...
    if (dialogState.wish) {
      // Update
      await db.WishRequest.update(dialogState.wish.id as string, {
        ...dataToSave,
      ...
      if (_createShift && dataToSave.position) {
        await createShiftFromWish(selectedDoctorId!, dateStr, dataToSave.position as string);
      }
      ...
      if (selectedDoctor) {
        logWishAction(
          `Eintrag aktualisiert (${dataToSave.status})`,
          selectedDoctor.name,
          dateStr,
          dataToSave.type as string,
        );
      }
    } else {
      // Create
      ...
      queryClient.invalidateQueries({ queryKey: ['wishes'] });
      if (selectedDoctor) {
        logWishAction('Eintrag erstellt', selectedDoctor.name, dateStr, dataToSave.type as string);
      }
    }
  };
  ...
  const handleDialogDelete = () => {
    if (dialogState.wish) {
      const wishToDelete = dialogState.wish;
      const dateToLog = dialogState.date;
      deleteWishMutation.mutate(wishToDelete.id as string, {
        onSuccess: () => {
          if (selectedDoctor) {
            logWishAction(
              'Eintrag gelöscht',
              selectedDoctor.name,
              format(dateToLog!, 'yyyy-MM-dd'),
              wishToDelete.type as string,
            );
          }
        },
      });
  ...
  return (
    <div className="container mx-auto max-w-7xl">
        ...
        >
          ...
          <TabsContent value="year" className="mt-0">
            {selectedDoctor ? (
              <WishYearView
                ...
                year={selectedYear}
                wishes={filteredDoctorWishes as any}
                shifts={doctorShifts as any}
                ...
                activeType={activeTab ?? undefined}
              />
            ) : (
          ...
          <TabsContent value="month" className="mt-0">
            <WishMonthOverview
              ...
              wishes={allWishes as any}
              shifts={allShifts as any}
              onDateChange={(year, month) => setViewDate(new Date(year, month, 1))}
              activeType={activeTab ?? undefined}
              onToggle={(date, _wish, doctorId) => {
      ...
      selectedDoctor ? (
        ...
        <div className="text-center py-12 text-slate-500">Bitte wählen Sie eine Person aus.</div>
      )}

      <WishRequestDialog
        isOpen={dialogState.isOpen}
        onClose={() => setDialogState({ ...dialogState, isOpen: false })}
        date={dialogState.date ? format(dialogState.date, 'yyyy-MM-dd') : undefined}
        wish={dialogState.wish}
        doctorName={selectedDoctor?.name}
        activePosition={activeTab ?? undefined}
        isReadOnly={!canEdit}
        isAdmin={isAdmin}
