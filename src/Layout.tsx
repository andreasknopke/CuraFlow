/**
 * CuraFlow — Layout Component
 *
 * Main application shell with collapsible sidebar navigation,
 * auth redirect logic, theme injection, and admin/user menus.
 *
 * @module Layout
 */
...
function LayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const servicesNavLabel =
    servicesCaption === 'Dienste' ? 'Dienstbesetzung' : servicesCaption;
  const rotationsNavLabel =
    rotationsCaption === 'Rotationen' ? 'Rotationsplaner' : rotationsCaption;

  const { user } = useAuth();

  // ── Init IndexedDB token storage ─────────────────────────────────────────
  ...
  // ── Dashboard alert (pending wishes / unviewed updates) ──────────────────
  ...
  const { data: hasDashboardAlert } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Regular user: check for unviewed updates
      if (!(currentUser as Record<string, unknown>).doctor_id) return false;
      const unviewed = await db.WishRequest.filter({
        doctor_id: (currentUser as Record<string, unknown>).doctor_id,
        user_viewed: false,
      });
      return (unviewed as unknown[]).length > 0;
    },
    enabled: isAuthenticated,
    refetchInterval: 30000,
  });

  // ── Auto-collapse sidebar after 3s ───────────────────────────────────────
  ...
  const isAuthPage =
    location.pathname.toLowerCase() === authLoginPath.toLowerCase();
  ...
  if (isEmbeddedSchedule) {
    return (
      <div
        className="min-h-screen bg-slate-50 font-sans text-slate-900 notranslate"
        translate="no"
      >
        ...
        <div className="px-3 py-4">
          {isAuthenticated ? (
            <nav className="space-y-1">
              ...
              <button
                onClick={() => {
                  setTicketDialogType(undefined);
                  setTicketDialogError(null);
                  setIsTicketDialogOpen(true);
                }}
